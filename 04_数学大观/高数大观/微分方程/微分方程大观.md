...
> - 例题按教学价值分详略：多数给完整解析，少数变体只给关键变形与答案（索引见 [[微分方程 MOC#例题索引]]）。
...
<div class="dp-cards">
<div class="dp-card"><b>二、概念与解的结构</b><span class="dp-chip">阶 / 解 / 通解 / 特解 / 初值</span><span class="dp-chip">线性算子的四条性质</span><span class="dp-chip">解的结构定理</span></div>
<div class="dp-card"><b>三、一阶方程</b><span class="dp-chip">可分离变量</span><span class="dp-chip">一阶线性</span><span class="dp-chip">Bernoulli</span><span class="dp-chip">换元法：整体 / 齐次 / 链式</span><span class="dp-chip">分式线性型</span><span class="dp-chip">Mdx+Ndy=0 四条出路</span></div>
<div class="dp-card"><b>四、高阶方程</b><span class="dp-chip">常系数齐次</span><span class="dp-chip">常系数非齐次 ＋ 待定系数</span><span class="dp-chip">可降阶</span><span class="dp-chip">Euler 方程</span><span class="dp-chip">二阶变系数换元</span><span class="dp-chip">差分方程（数三）</span></div>
<div class="dp-card"><b>五、综合与应用</b><span class="dp-chip">导数定义</span><span class="dp-chip">导数几何应用</span><span class="dp-chip">定积分与变限积分</span><span class="dp-chip">二重积分变区域</span><span class="dp-chip">应用建模</span></div>
</div>

> [!tip] 读图的重点
> 「二、概念与解的结构」是**公理层**：所有线性方程共用同一套"解的结构"，而「四」里的待定系数法完全建立在 $y=y_h+y_p$ 上——这是本章唯一的"依赖关系"。
> 「三」「四」是**操作层**：一阶看"能不能换来换去"，高阶看"是常系数还是变系数"。
...
> [!example] 一个例子解决一类题
> 已知 $y_1,y_2,y_3$ 都是 $y''+p(x)y'+q(x)y=f(x)$ 的解，请分别判断
> $y_1-y_2$、$y_1+y_2-y_3$、$2y_1-y_2-y_3$ 是什么。
>
> > [!success]- 点开看解析
...
顺着"否"一路往下问，**问到自己能答"是"的那一行，就按右边的结论去做**。

<div class="dp">
<div class="dp-row"><span class="dp-ask">① 能分离 x 与 y?</span><span class="dp-yes">是 →</span><span class="dp-ans">可分离变量 · 3.2</span></div>
<div class="dp-no"></div>
<div class="dp-row"><span class="dp-ask">② 右端只含 y/x?</span><span class="dp-yes">是 →</span><span class="dp-ans">齐次方程 u = y/x · 3.5.2</span></div>
<div class="dp-no"></div>
<div class="dp-row"><span class="dp-ask">③ 反复出现整体 ax+by+c?</span><span class="dp-yes">是 →</span><span class="dp-ans">整体换元 · 3.5.1</span></div>
<div class="dp-no"></div>
<div class="dp-row"><span class="dp-ask">④ 是两个一次式的商?</span><span class="dp-yes">是 →</span><span class="dp-ans">分式线性型 · 3.7</span></div>
<div class="dp-no"></div>
<div class="dp-row"><span class="dp-ask">⑤ 能整理成 y′ + P(x)y = Q(x)?</span><span class="dp-yes">是 →</span><span class="dp-ans">一阶线性 · 3.3</span></div>
<div class="dp-no"></div>
<div class="dp-row"><span class="dp-ask">⑥ 能整理成 y′ + Py = Qy<sup>α</sup>?</span><span class="dp-yes">是 →</span><span class="dp-ans">Bernoulli · 3.4</span></div>
<div class="dp-no"></div>
<div class="dp-row"><span class="dp-ask">⑦ 能写成 M dx + N dy = 0?</span><span class="dp-yes">是 →</span><span class="dp-ans">四条出路 · 3.6</span></div>
<div class="dp-no"></div>
<div class="dp-row"><span class="dp-ask">⑧ 把 y 当函数实在做不动</span><span class="dp-yes">→</span><span class="dp-ans dp-sub">倒换 dx/dy · 3.6.3</span></div>
</div>

> [!tip] 怎么用这张图
> 每行右边那个胶囊就是"命中后该做什么"，后面的数字是它在本文的节号——
> 把这一列节号串起来（3.2 → 3.3 → 3.4 → 3.5 → 3.6 → 3.7），正好是下面六节的顺序。
...
> [!example] 例 3 与例 9：两个"不标准"的整体换元
> - **例 3** $y'=\dfrac{y-x+1}{y-x+5}$
> - **例 9** $y'=y^2+2(\sin x-1)y+\sin^2x-2\sin x-\cos x+1$
>
> > [!success]- 点开看解析
> > **例 3** 右端是整体 $y-x$ 的函数，先**拆项**再换元：写成 $y'=1-\dfrac{4}{y-x+5}$，
> > 令 $u=y-x+5$，得 $u'=-\dfrac4u$，即 $u\,\mathrm du=-4\,\mathrm dx$，故 $u^2+8x=C$，通解 $(y-x+5)^2+8x=C$。
> > **例 9** 先把右端**配方**成 $y'=(y+\sin x-1)^2-\cos x$，再令 $u=y+\sin x-1$，
> > 得 $u'=u^2$（纯可分离），解得 $-\dfrac1u=x+C$，通解 $y=1-\sin x-\dfrac{1}{x+C}$。
> >
> > 两例的"框框"都不是 $ax+by+c$ 的标准形状，但**本质一样**：都要先找一个"导数能被整体表示"的组合。
...
> [!example] 例 12：又一个"平方和"型整体换元
> $2yy'=e^{\frac{x^2+y^2}{x}}+\dfrac{x^2+y^2}{x}-2x$
>
> > [!success]- 点开看解析
> > 注意 $u=x^2+y^2$ 时 $u'=2x+2yy'$，于是原方程恰好变成 $u'=e^{u/x}+\dfrac ux$——
> > 这是 $v=u/x$ 的齐次方程，解出 $-e^{-\frac{x^2+y^2}{x}}=\ln\lvert x\rvert+C$。
> >
...
> [!note] 它和「整体换元」的区别
> 本节的题型是 $\dfrac{dy}{dx}=f\!\left(\dfrac{ax+by+c}{a_1x+b_1y+c_1}\right)$（六个常数）。
> 与 3.5.1 整体换元相比：**分子分母各是一个一次式，换元换不掉——得先把坐标系平移一下。**
...
> > $$
> >
> > **另一条路：这题其实是全微分方程，可以秒杀。** 把方程写成 $M\,\mathrm dx+N\,\mathrm dy=0$ 并查 $M_y=N_x$：
> > $$
...
> >
> > **⚠️ 注意 $\mathrm dy$ 项的符号。** 若误写成 $(x+y-2)\mathrm dx+(x-y+4)\mathrm dy=0$，凑出来会是 $+8y$，
> > 它对应的是 $y'=-\dfrac{x+y-2}{x-y+4}$，**与原题不是同一个方程**。
...
> [!example] 例 6 与例 7：两个变体
> - **例 6** $(x-y-1)\mathrm dx+(4y+x-1)\mathrm dy=0$
> - **例 7**（15 届初赛）$(x^2+y^2+3)\dfrac{dy}{dx}=2x\!\left(2y-\dfrac{x^2}{y}\right)$
>
> > [!success]- 点开看解析
> > **例 6** 先化为 $\dfrac{dy}{dx}=\dfrac{y-x+1}{4y+x-1}$，令 $X=x-1$、$Y=y$（这题的平移就是 $x\to x-1$）
> > 直接消掉常数，得 $\dfrac{dY}{dX}=\dfrac{Y-X}{4Y+X}$。再令 $u=\dfrac YX$，得到
> > $\dfrac{4u+1}{4u^2+1}\,\mathrm du=-\dfrac{\mathrm dX}{X}$——注意这里 $4u^2+1$ **不能再分解**，
> > 积分出来是 $\ln$ 与 $\arctan$ 的混合项，通解 $\ln\left[(x-1)^2+4y^2\right]+\arctan\dfrac{2y}{x-1}=C$。
> >
> > **例 7** 先整理成 $\dfrac{y\,\mathrm dy}{x\,\mathrm dx}=\dfrac{2(2y^2-x^2)}{x^2+y^2+3}$，再令 $u=x^2$、$v=y^2$，
...
| 大类 | 子类 | 节号 | 适用卷种 |
|---|---|---|---|
| **常系数** | 二阶常系数线性齐次 | [[#4.2 常系数齐次：特征方程\|4.2]] | 数一、二、三 |
| | 二阶常系数线性非齐次 | [[#4.3 常系数非齐次与待定系数法\|4.3]] | 数一、二、三 |
| | 高阶常系数线性齐次 | [[#4.2.3 高阶：根型 → 通解的统一规则\|4.2.3]] | 数一、二、三 |
| | 差分方程 | [[#4.7 差分方程（数学三）\|4.7]] | **数三** |
| **二阶变系数** | 二阶可降阶（缺 $y$ / 缺 $x$） | [[#4.4 可降阶的高阶方程（数学一、二）\|4.4]] | 数一、二 |
| | Euler 方程 | [[#4.5 Euler 方程（数学一、二）\|4.5]] | 数一、二 |
| | 三角换元 / 反函数 / 题干给的换元 | [[#4.6 二阶变系数：处理原则\|4.6]] | 数一、二 |

> [!tip] 判据只有两步
> 先问**是不是常系数**（系数里有没有 $x$）：是 → 一律走**特征方程**；
> 不是 → 再看**缺哪一项**（缺 $y$ 还是缺 $x$）、**是不是 Euler**、**题干给没给换元**。
