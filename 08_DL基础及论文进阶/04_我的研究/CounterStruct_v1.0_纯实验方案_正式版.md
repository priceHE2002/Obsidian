# CounterStruct v1.0 纯实验方案（正式版）

## 1. 项目定义

**CounterStruct: Counterfactual Structural Adaptation for Fixed-Capacity Continual Learning**

CounterStruct 在固定 active connectivity capacity 下，对 Transformer FFN 的 connectivity graph 进行动态 exact contiguous 2:4 重构；其核心不是“动态稀疏”本身，而是对完整结构动作进行 optimizer-aware counterfactual scoring，并通过 empirical locality calibration 检验该评分的预测有效范围。
...
CounterStruct v1.0 的核心实验不以“动态稀疏本身”为主要贡献，而围绕两个必须被直接检验的机制问题展开：

1. **完整结构动作的反事实评分是否优于分离式 prune / grow heuristic？** 主要通过 CounterStruct、IPGH 与 SRigL-style Exact-2:4 的同预算比较回答。IPGH 是专门构造的 matched independent prune/grow heuristic baseline，不代表任何旧项目版本。
...
在上述主线之外，CounterStruct v1.0 进一步检验：
...
重新编号。正文、表格、日志、配置与最终论文中均不使用任何旧项目名称或旧版本号描述 CounterStruct。

IPGH（Independent Prune/Grow Heuristic）仅作为结构动作分解式 heuristic baseline，用于回答 complete-action scoring 是否真正带来增益；它不是 CounterStruct 的前一版本。

标题不包含 “Large Language Models”，因此 Qwen3-8B 单 seed 实验只作为 mechanism scale-direction validation，不承担 LLM-scale claim。
...
Qwen3-8B 只用于 mechanism scale-direction validation，不参与 CounterStruct hyperparameter development。
...
\[
\boxed{
\text{CounterStruct + 64 exemplars/task}
}
\]
...
# 26. CounterStruct v1.0 Pseudocode
...
该 composition 与三种长序列 order 均已有公开 continual-learning 文献协议；CounterStruct 的 primary long-horizon stream 使用 Order-4，不根据实验结果选择 order。
...
CounterStruct-specific mutation schedule、\(\rho_{\max}\)、cooldown、probe budget、historical consolidation budget 与 utility formula 全部继承 TRACE Order-1 development 后冻结的配置。

**Long-CL-15 上禁止重新调 CounterStruct-specific hyperparameter。**
...
Order-5 / Order-6 仅用于 secondary order extension，不进入 CounterStruct hyperparameter selection。
...
\[
\Delta^{(o)}_{\mathrm{CounterStruct-SRigL}}
=
Metric^{(o)}_{\mathrm{CounterStruct}}
-
...
- Static / SRigL / IPGH / CounterStruct：
  \[
...
CounterStruct 与 external baseline 的 paired contrast 只使用公共 seed subset：
...
除 architecture-induced target-layer mapping 外，全部继承已经 freeze 的 Qwen3-1.7B CounterStruct 配置：
...
只训练与 CounterStruct 相同的 target FFN matrices，但不施加 sparsity。
...
## B3 — IPGH (Independent Prune/Grow Heuristic)

构造与 CounterStruct 预算匹配的分离式 prune / grow heuristic：
...
以保证与 CounterStruct 的 replay policy 一致。

IPGH 不使用 complete-action counterfactual utility，也不使用 CounterStruct 的 joint keep-vs-swap scoring；其作用是隔离“完整结构动作评分”相对于独立 prune/grow 排名的增益。
...
禁止为了“看起来更公平”而把 external method 改造成 CounterStruct 的 parameterization。
...
CounterStruct 的 primary general-capability decomposition 仍以 TRACE-8 Order-1 为准。
...
Calibration 不用于 CounterStruct hyperparameter selection。
...
# 51. Ablation A4 — IPGH Independent Prune/Grow Heuristic
...
CounterStruct 与 external baseline 的 pairwise contrast 必须限制在：
...
禁止把 CounterStruct 的 5-seed mean 与某 external method 的 3-seed mean 直接当成 paired effect。
...
1. CounterStruct vs SRigL-style Exact-2:4；
2. CounterStruct vs IPGH。
...
1. CounterStruct vs **OSFT**；
2. CounterStruct vs Meta-UCF。
...
- CounterStruct vs Any-SSR；
- CounterStruct vs PaRSP；
- CounterStruct vs GORP；
- CounterStruct vs TreeLoRA；
- CounterStruct vs O-LoRA。
...
| IPGH | 5 | 0 | none | | | | | | | |
| **CounterStruct** | **5** | **0** | **none** | | | | | | | |

`Replay`、`Task-ID / Router` 对 external baseline 由 code audit 后填写，不预先假定。

CounterStruct 与 external method 的显式 effect table 使用 seeds 42/43/44 重新聚合。
...
CounterStruct 的 `Persistent-state Growth / Task = 0` 仅指 \((P,H)\) 等固定-size persistent tensors 的**张量尺寸**不随 task 数增加；数值内容会持续更新。
...
因此 Qwen3-8B mechanism scale-direction validation 通过只修改最后 2 层，将 CounterStruct candidate-level persistent historical memory 保持在与 1.7B primary experiment 相同量级。
...
CounterStruct efficiency claim 只允许建立在：
...
1. 用 vectorized CounterStruct scoring 计算 \(U\)；
2. clone keep branch；
3. clone swap branch；
...
## 68.1 CounterStruct Development

唯一 CounterStruct development stream：
...
**Seq-GLUE-7、Long-CL-15、TRACE Order-2、Llama backbone、Qwen3-8B 均不是 CounterStruct development stream。**
...
如果需要修改公式，则版本升级，不得继续称为 frozen CounterStruct v1.0。
...
任何 freeze 后的方法级公式变化都要求创建新版本，不得用 CounterStruct v1.0 final-test result 反向调参。
...
## G3 — CounterStruct Complete-Action Counterfactual Operator

Development seed 比较：

\[
\boxed{
\text{CounterStruct vs IPGH}.
}
\]

要求至少在 ACC / Forgetting 之一出现稳定改善，同时 AG 无明显下降。

若 CounterStruct 相对 IPGH 完全无改善，则停止扩展 CounterStruct 大规模实验，保留 failure analysis。
...
\operatorname{sign}
\left(
\Delta_{\mathrm{CounterStruct-IPGH}}
\right)
}
\]
...
\operatorname{sign}
\left(
\Delta_{\mathrm{CounterStruct-SRigL}}
\right)
}
\]
...
Order-2 中，CounterStruct vs SRigL 的 ACC / Forgetting 至少一个主要稳定性指标保持与 Order-1 相同改善方向，且 AG 不出现明显 collapse。
...
15. freeze CounterStruct method config；
16. TRACE Order-1 Static / SRigL / IPGH / CounterStruct seeds 42–46；
...
36. Qwen3-8B TRACE Static / SRigL / CounterStruct seed42 with zero retuning；
...
- [ ] Static / SRigL / IPGH / CounterStruct 共用对应 backbone 的 \(M_0\)
...
- [ ] Long-CL no CounterStruct retuning
