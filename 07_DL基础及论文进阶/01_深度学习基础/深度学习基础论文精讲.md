...
按技术发展脉络排列，覆盖从深度学习革命（AlexNet, 2012）到现代 LLM 基础架构的核心演进。共 **23 篇论文**（LoRA 已移至 [[03_训练基础设施论文精讲|03_训练基础设施]]），分为四大主题：**CNN 架构演进**（5 篇）、**归一化技术**（4 篇）、**Transformer 与视觉基础模型**（8 篇）、**生成模型与 VLA 训练基础设施**（6 篇）。
...
输入图像 → [视觉编码器] → [Transformer骨干/LLM] → [动作解码器] → 机器人动作
         ...
         (Alex→VGG→        (Attention→          Matching/CVAE
         ResNet→ViT→        BERT/GPT→          (VAE→DDPM→Flow
         DINOv2)            Llama 2)            Matching→DiT)
                                                     ↑
                                               [CVAE/ACT]
                                               [LoRA微调]
                                               [AdamW优化]
...
2013  VAE             ─ 变分自编码器，重参数化技巧+ELBO，生成模型奠基作
...
这一批论文直接对应 VLA 训练和推理中的核心组件。与前二批不同，这些论文的实践价值等同于理论价值——它们定义了 VLA 研究者每天使用的工具链。

### 18. [[VAE]]（2013，ICLR 2014）

**核心地位：深度学习时代生成模型的奠基作——重参数化技巧和 ELBO 优化改变了整个领域**

**背景与动机：** 2013 年以前，训练具有连续潜变量的概率生成模型需要 MCMC 采样（慢且难规模化），或对每个数据点独立优化变分参数（无法批量训练）。VAE 用一个神经网络（encoder/识别模型）**同时对所有数据点做摊销推断**，端到端用 SGD 优化。

**核心贡献：**
- **重参数化技巧**：$z = \mu_\phi(x) + \sigma_\phi(x) \odot \epsilon$，$\epsilon \sim \mathcal{N}(0, I)$——让采样对 encoder 参数可微
- **SGVB 估计器**：在重参数化的基础上构造 ELBO 的低方差、可微 Monte Carlo 估计
- **AEVB 算法**：encoder（识别模型）和 decoder（生成模型）联合训练，SGD/Adagrad 优化
- **ELBO 天然正则化**：$-D_{KL}(q_\phi(z|x) \parallel p(z))$ 自动充当正则项，无需手动调正则化超参数

**为什么对 VLA 重要：**
- **CVAE（条件 VAE）被 ACT 用于动作生成**——同一 encoder 学习动作的 latent $z$，推理时从先验采样生成多样化动作
- 重参数化技巧成为所有"可微采样"场景的默认工具（扩散模型的 denoising、强化学习的 stochastic policy）
- ELBO 框架是理解所有变分/扩散模型损失函数的基础语言——DDPM 的损失、Flow Matching 的 CFM 损失都能追溯到 ELBO

**关键启示：** VAE 本身在图像生成中已被扩散模型超越，但它的三个核心概念（重参数化、摊销推断、ELBO）是所有现代生成模型的基础——这些概念必须理解，VAE 的工程细节不需要死记。

---

### 19. [[Scaling Laws]]（2020）
...
### 21. [[Flow Matching]]（2022，ICLR 2023）
...
变分自编码器 + 重参数化
    ...
    │    条件VAE
    │
    └──→ 重参数化思想 → DDPM / Flow Matching 的采样过程
          │
...
6. **生成式 AI 与判别式融合**（VAE → DDPM → Flow Matching → DiT——生成模型被用作策略/规划器）
...
```
理论层：     VAE → Attention → BERT/GPT → DDPM → Flow Matching → Scaling Laws
                 ↓        ↓         ↓         ↓           ↓              ↓
架构层：     ACT    ViT/CLIP    Llama 2    DiT        π0         OpenVLA 7B vs 1B
                 ↓        ↓         ↓         ↓           ↓              ↓
训练层：     CVAE   DINOv2      RMSNorm    AdaLN    Flow Action    LoRA/QLoRA
                 ↓        ↓         ↓         ↓           ↓              ↓
VLA 落地：  ACT    OpenVLA     OpenVLA    π0/GR00T   π0/GR00T     OpenVLA 微调
          动作生成 空间编码器   LLM骨干  Action Expert 动作生成  消费级 GPU 可训
```
...
graph TD
    ...
    subgraph "2013-2019 - 生成模型与Transformer诞生"
        VAE["VAE (2013) - ICLR2014"]
...
9. **VAE** — 重参数化技巧、ELBO、摊销推断——所有生成模型的基础概念
10. **Scaling Laws** — 为什么模型越大能力越强？涌现的数学基础
11. **RMSNorm** — LayerNorm→RMSNorm 的演化逻辑

**学完 Level 2 你应该能回答：**
- Adam 的二阶矩 v_t 做了什么？为什么 SwiGLU 比 ReLU 需要更小学习率？
- "预训练+微调"为什么有效？MLM 和因果LM 各自适合什么场景？
- 重参数化技巧解决了什么问题？ELBO 的两项分别对应什么？
- Scaling law 的幂律指数 α 大约是多少？这意味着什么？

### Level 3：视觉基础模型 + 生成模型（约 2 周）

12. **ViT** — Transformer 怎么进入 CV，为什么需要大数据预训练？
13. **CLIP** — 视觉和语言怎么对齐到同一空间？
14. **DDPM** — 扩散模型的前向/反向过程，score function 的直觉
15. **Flow Matching** — 从随机路径到最优传输直线，为什么更快？
16. **DiT** — Transformer 怎么做扩散生成？AdaLN 是什么？
17. **DINOv2** — **重点读**——OpenVLA 的空间编码器
...
18. **Swin Transformer** — 层次化架构 + 窗口注意力
19. **MAE** — 图像上 mask 75%，为什么比 BERT 高？
20. **Llama 2** — **必读**——理解 OpenVLA 的每层内部结构（RMSNorm + SwiGLU + RoPE + GQA）
...
| ACT | ResNet Encoder + CVAE | **ResNet**, **BatchNorm**, **Adam**, **VAE** |
...
深度学习基础 (本文件夹 23 篇)
    ↓
    ├── Level 1-2: CNN + 归一化 + Transformer 基础 + VAE
    │   AlexNet→ResNet→BatchNorm→LayerNorm→Attention→Adam→ScalingLaws→VAE
    │
    ├── Level 3: 视觉基础模型 + 生成模型
...
| 11 | [[VAE]] | MLP Encoder+Decoder (~几M) | ~1GB (MNIST) | ~0.1GB | CPU 可训，GPU 任意 |

### ⚠️ 条件可行（需较大 GPU 或量化）

| # | 论文 | 条件 | 训练 VRAM | 推理 VRAM | 备注 |
|---|------|------|-----------|-----------|------|
| 12 | [[Attention Is All You Need]] | Big (213M) | ~4GB | ~1GB | Base 模型只需 ~2GB |
| 13 | [[BERT]] | BERT-Large (340M) | ~6GB (fp32) | ~2GB | BERT-Base 仅需 ~3GB |
| 14 | [[GPT]] | GPT-1 (117M) | ~2GB | ~0.5GB | GPT-2 1.5B 需 ~10GB |
| 15 | [[Swin Transformer]] | Swin-B (88M) | ~6GB | ~2GB | 窗口注意力节省显存 |
| 16 | [[CLIP]] | ViT-B/32 (151M) | ~4GB | ~1GB | ViT-L/14 需 ~10GB |
| 17 | [[MAE]] | ViT-L (307M) Encoder | ~8GB | ~2GB | 预训练需较多计算 |
| 18 | [[DINOv2]] | ViT-g/14 (1.1B) | ❌ 不可训练 | ~6GB 推理 | 仅推理可用，需下载预训练权重 |
| 19 | [[DiT]] | DiT-XL/2 (675M) | ~20GB | ~8GB | RTX 3090 24GB 可训练，RTX 4070 Ti 需量化 |
| 20 | [[DDPM]] | U-Net (CIFAR) | ~3GB | ~1GB | 图像级可行，视频扩散需更多 |
| 21 | [[Flow Matching]] | 同 DDPM/DiT | 取决于架构 | 取决于架构 | 推理步数少（~10步），效率高 |
| 22 | [[Scaling Laws]] | — | 元论文 | — | 理解理论，无需 GPU |
...
| # | 论文 | 配置 | 推理 VRAM | 训练需求 |
|---|------|------|-----------|---------|
| 23 | [[Llama 2]] | 7B (4-bit 量化推理) | ~6GB (4-bit) | 7B 全量需 ~56GB (8×A100) |

### 重点说明

- **CNN 架构**（AlexNet~DenseNet）在消费级 GPU 上完全可跑，建议实际运行一遍 ImageNet 训练以建立直觉
- **VAE** 极轻量——MNIST 级 VAE 在 CPU 上即可训练（论文用 CPU 跑，约 20-40 分钟/百万样本），是理解生成模型的最佳入门实验
...
| 18 | [[VAE]] | 用重参数化技巧实现可微的潜变量采样，ELBO 框架 + 摊销推断成为所有生成模型的基础语言 |
| 19 | [[DDPM]] | 用前向加噪+反向去噪的扩散框架实现高质量生成，Score Function 解释为扩散策略的稳定性奠基 |
| 20 | [[Flow Matching]] | 用确定性的最优传输直线路径替代 DDPM 的随机路径，实现 ~10 步快速生成，成为 π0 的动作生成核心 |
| 21 | [[DiT]] | 证明 U-Net 不是扩散生成的必需品——ViT + AdaLN 更优，成为 π0 Action Expert 的架构蓝本 |
| 22 | [[Scaling Laws]] | 用幂律关系建立模型性能随规模增长的数学框架，解释了大模型的涌现能力 |
| 23 | [[Llama 2]] | 将 RMSNorm + SwiGLU + RoPE + GQA 四大组件工程化为开源 LLM 标杆，成为 OpenVLA 的骨干 |
