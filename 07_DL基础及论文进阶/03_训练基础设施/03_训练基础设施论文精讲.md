...
本专题按**训练效率的八大基础设施方向**排列 21 篇论文，覆盖从 2016 年 Gradient Checkpointing 到 2025 年 Muon 扩展，时间跨近 10 年的核心使能技术。Wan et al. (TMLR 2024)、Bai et al. (2024)、Duan et al. (2024)、Liu et al. (2024)、Wang et al. (2025) 五篇权威综述均将这些技术归为高效 LLM 训练的核心支撑。
...
2024 ─ GaLore (Zhao et al.) · Muon (Keller Jordan, Blog)
2025 ─ Muon Scalability / MuonClip (Moonshot AI, arXiv:2502.16982)
```
...
### 11. [[Muon]]（2024，Blog + arXiv:2502.16982）

**核心地位：** 第一个将 Newton-Schulz 矩阵正交化应用于优化器的博客驱动创新。Keller Jordan 凭借 Muon 在 NanoGPT/CIFAR-10 速度训练中创下世界纪录后入职 OpenAI。Moonshot AI 将 Muon 扩展至 16B MoE 生产级训练（Moonlight），证明了 ~2× 计算效率提升。

**背景与动机：** AdamW / Lion 均逐元素处理梯度，将权重矩阵视为独立标量。但神经网络的 hidden layer 权重本质上是线性映射——将向量空间映射到另一个向量空间。Keller Jordan 观察到 SGD-momentum 和 Adam 对 2D 参数产生的更新矩阵条件数极高（几乎是低秩的），少数方向主导了所有神经元的更新。

**核心贡献：** Muon = SGD+Nesterov动量 + Newton-Schulz 正交化：

1. **动量累积**：$M_t = \beta M_{t-1} + G_t$，再用 Nesterov 风格计算 $G' = G_t + \beta M_t$
2. **Newton-Schulz正交化**：用五次多项式迭代 $X_{k+1} = aX_k + b(X_k X_k^\top)X_k + c(X_k X_k^\top)^2 X_k$，系数 $(3.4445, -4.7750, 2.0315)$ 故意不收敛（最大化小奇异值的放大速度），在 bf16 中只需 5 步即可近似 $UV^\top$
3. **参数更新**：$W_{t+1} = W_t - \eta \cdot O$

Moonshot AI 的两项关键扩展使之可规模化：
- **权重衰减**：$\lambda=0.1$（防止 bf16 范围溢出）
- **RMS-一致缩放**：$O \gets 0.2 \cdot O \cdot \sqrt{\max(d_{\text{out}}, d_{\text{in}})}$，使 AdamW 超参数可直接迁移

**关键发现：**
- NanoGPT speedrun 世界纪录（比 AdamW 快 1.35×），CIFAR-10 94% 准确率仅用 2.6 A100-seconds（原纪录 3.3s）
- 1.5B Transformer → GPT-2 XL 质量：Muon 10h vs AdamW 13.3h（8×H100）
- Moonlight (16B MoE, 5.7T tokens)：~2× 计算效率，MMLU 60.4 vs DeepSeek V3-Small 53.3
- MuonClip 使 Kimi K2 (1T MoE) 训练零崩溃

**历史意义：** 2024 年底最重要的优化器创新。虽仅以博客发布，但通过"竞争性任务"证据标准（NanoGPT speedrunning 持续 12 次刷新无人用 AdamW 打破）建立了独特的可信度。Moonshot AI 的生产验证表明 Muon 不是学术玩具——是有望替代 AdamW 的生产级优化器。

**关键启示：** Muon 仅用于 2D hidden 权重；embedding / head / bias / LayerNorm 仍需 AdamW（Hybrid 配方）。Newton-Schulz 迭代的 FLOP 开销 <1%（$T \cdot m / B$）。Q、K、V 需分开应用（不合并）。预训练场景使用 Muon，微调保持 AdamW。

---

## 七、参数高效微调（PEFT）：冻结预训练的学习

### 12. [[Adapter]]（2019，ICML）
...
### 趋势 6：从"逐元素自适应"到"矩阵级结构化更新"

2023-2025 年优化器设计的范式转变。Lion 用 $\text{sign}(\cdot)$ 统一了更新幅度；Muon 更进一步，用 Newton-Schulz 迭代将整个更新矩阵正交化。两者均放弃了 Adam 的逐参数自适应学习率——矩阵级结构化更新正在成为优化器创新的新方向。
...
- [[Muon]]：更新矩阵的条件数极高（几乎是低秩的），正交化使得罕见方向获得同等更新幅度
...
15. [[Lion]] — 优化器的进化搜索
16. [[Muon]] — 矩阵正交化优化器的最新前沿
17. [[SparseGPT]] / [[Wanda]] — LLM 后训练剪枝两种范式
18. [[FSDP]] — PyTorch 原生分布式方案
19. [[GaLore]] — 梯度低秩投影的前沿
...
- Newton-Schulz 迭代为何能在 bf16 中稳定运行？Muon 的正交化与 Lion 的 sign 操作有什么相同效果？
...
| 11 | Muon | 预训练（hidden 层） | 8GB（需 bf16） | RTX 4090 24GB |
...
| 11 | Moonlight/Muon (7B+) | 预训练 | 需 bf16 GPU | 建议 A100/4090 |
...
| 11 | Muon | Newton-Schulz 正交化 SGD 动量 = 矩阵级更新 + ~2× 计算效率 (Moonshot AI) |
| 12 | Adapter | 瓶颈模块插入 Transformer 子层 = PEFT 的原始方案（有推理延迟） |
