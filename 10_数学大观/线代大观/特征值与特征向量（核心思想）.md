...
这句话要**反着读**才是本质。**正着读**很平凡：对角矩阵作用在单位坐标向量上，第 $i$ 个坐标只被第 $i$ 个对角元乘一下，别的坐标"碰不到"它，所以 $De_i=d_ie_i$ 是"白给"的。

而**反着读**：一般矩阵看起来复杂（会扭方向），是因为我们站在了跟它"天然方向"错位的坐标系里看它。换一套坐标、让坐标轴对准特征方向，矩阵的作用就变成"各方向分别伸缩"：

$$\boxed{P^{-1}AP=D}\quad\Longleftrightarrow\quad \text{换成特征方向看，}A\text{ 只做"各方向分别伸缩"}$$

此时 $D$ 的主对角元就是特征值，$P$ 的列向量（特征向量）就是新坐标系里的"单位坐标向量"。

> [!tip] 直观想象
> 一块弹性布料沿**纹理方向**拉伸，处处均匀、不会扭曲；但若斜着描述这个拉伸，就不得不处理交叉项，显得复杂。特征方向就是这块布料的"纹理"，标准坐标只是我们习惯拿的那把"斜尺子"。

| 读法 | 内容 |
|---|---|
| **正着读** | 对角矩阵 → 特征值就是对角元、特征向量就是单位向量（最平凡的实例） |
| **反着读** | 可对角化矩阵 → 找到特征方向、换成这套坐标 → 就变成对角矩阵（所有努力的目的） |

> [!warning] 别理解过头
> 1. 并非所有矩阵都能"完全扳直"：凑不出 $n$ 个线性无关特征向量时，只能退到约当标准形，思想一脉相承：**尽力找一套让矩阵最简单的坐标**。
> 2. "标准单位坐标向量"应理解为"特征方向上的单位向量"：对对角矩阵二者恰好重合；对一般可对角化矩阵，特征是斜的，它的"单位向量"就是特征向量 $\alpha$。
