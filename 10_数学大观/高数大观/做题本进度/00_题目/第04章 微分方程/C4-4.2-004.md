...
备注:
---
# 4.2-004 · 微分方程解的极限及渐近线

> [!question] 题目（程序自动提取，公式可能有偏差）
> ′′ ′ (x∈[0,+∞)) 设 是常数，连续函数 满足 f(x)=b，y 是微分方程 a>0 f(x) 
> lim =y(x) y +ay =f(x) x→+∞ ′ ′′ 的解，则 ， lim y (x)= lim 
> y (x)= x→+∞ x→+∞
>
> 题源：25 版 660 数一二三第 77 题　|　题源类型：660　|　页码：p.307　|　模块：4.2 微分方程解的极限及渐近线
