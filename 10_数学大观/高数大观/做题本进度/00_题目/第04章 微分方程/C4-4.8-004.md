...
备注:
---
# 4.8-004 · 微分方程解的定积分（应用）

> [!question] 题目（程序自动提取，公式可能有偏差）
> xf′(x)−2f(x)=−4x，且由曲线 设函数 满足 与直线 以及 轴所围成的平面图形绕 f(x) y 
> =f(x) x=1 x x 轴旋转一周所得旋转体的体积最小，则 f(x)= 5x2−4x 4x−5x2−2 
> 5x2−4x−2 4x−5x2 A. B. C. D. 微分方程解的反常积分
>
> 题源：26 版 660 数一二三第 219 题　|　题源类型：660　|　页码：p.310　|　模块：4.8 微分方程解的定积分（应用）
