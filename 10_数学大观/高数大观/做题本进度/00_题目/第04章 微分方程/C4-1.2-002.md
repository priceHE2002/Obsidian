...
> y′′+p(x)y′+q(x)y 的三个线性无关的解，f(x)̸=0，则该方程 【5】设 是微分方程 y (
> x),y (x),y (x) =f(x) 1 2 3 的通解为（ ）。 (x)+(1−2C A. C y (
> x)+C y (x)+y (x) B. C y )y (x)+C y (x) 1 1 2 2 3 1 1 1
>  2 1 3 −C C. (C )y (x)+C y (x)+y (x) D. C y (x)+C y (x
> )+C y (x)(C +C +C =1) 1 2 1 2 2 3 1 1 2 2 3 3 1 2 3
>
> 题源：880 第六章基础选择 5　|　题源类型：880　|　页码：p.278　|　模块：1.2 二阶及以上
