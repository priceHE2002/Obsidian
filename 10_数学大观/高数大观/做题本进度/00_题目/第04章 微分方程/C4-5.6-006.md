...
> [0,+∞) t∈ [0,+∞)，直线 设 是区间 上具有连续导数的单调增加函数，且 f(0)=1。对任意的
>  =0， f(x) x x=t，曲线 以及 轴所围成的曲边梯形绕 轴旋转一周生成一旋转体。若该旋转体的侧面面
> 积在 y =f(x) x x 数值上等于其体积的 倍，求函数 的表达式。 2 f(x)
>
> 题源：2008 数二　|　题源类型：真题　|　页码：p.324　|　模块：5.6 定积分应用（面积体积）
