...
> 设 为 平(cid:17)面上过点 的一条单调增加的曲线，其上任意一点 处的切线与 y =y(x) (x>0
> (cid:16)) xOy (1,0) P(x,y) y π 等于原点与 的连线 与该切线的夹角。 轴正方向
> 的夹角 P OP α 0<α< 2 （I）求曲线 的方程； y =y(x) √ 0<x⩽ （II）若某种镜面
> 的表面为由曲线 绕 轴旋转一周所得旋转曲面，求该镜面的表面积。 y =y(x) 3 y
>
> 题源：900 第六章数二 B 类 28　|　题源类型：900　|　页码：p.317　|　模块：5.4 5.4
