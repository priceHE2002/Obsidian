...
> y′′+py′+qy 设 与 是二阶线性微分方程 的两个解，y 与 是二阶线性微分方程 y (x) y (x
> ) =f(x) (x) y (x) 1 2 3 4 y′′+py′+qy y′′+py′+qy =f(x)−
> g(x) 的两个解，则下列函数中，一定是二阶线性微分方程 的解 =g(x) 的是 (x)−2y (x)−y 
> (x)−y (x)−2y A. y (x)+2y (x) B. 2y (x)+y (x) 1 2 3 4 1
>  2 3 4 (x)−y (x)−y (x)−2y (x)−2y C. 2y (x)+2y (x) D. y
>  (x)+y (x) 1 2 3 4 1 2 3 4
>
> 题源：26 版 660 数一二三第 212 题　|　题源类型：660　|　页码：p.279　|　模块：1.2 二阶及以上
