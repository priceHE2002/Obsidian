...
备注:
---
# 4.2-003 · 微分方程解的极限及渐近线

> [!question] 题目（程序自动提取，公式可能有偏差）
> [0,+∞) 【例 4.3】设 在 上连续，且 f(x) lim f(x)=b. x→+∞ y′+ay 若 
> 为微分方程 的解，求 (I) y =y(x) =f(x) (a>0) lim y(x). x→+∞ ′ ′′
>  y′′+ay′ 若 为微分方程 的解，求 与 (II) y =y(x) =f(x) (a>0) lim y
>  (x) lim y (x). x→+∞ x→+∞
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.307　|　模块：4.2 微分方程解的极限及渐近线
