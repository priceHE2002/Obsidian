...
> x2+y2 2 h(t)− 设有一高度为 h(t)（t 为时间）的雪堆在融化过程中，其侧面满足方程 （设长度
> 单位 z = h(t) 为厘米，时间单位为小时），已知体积减少的速率与侧面积成正比（比例系数为 0.9），问
> 高度为 厘米的 130 雪堆全部融化需多少小时？
>
> 题源：2001 数一（数学一专享）　|　题源类型：真题　|　页码：p.329　|　模块：5.9 物理及实际应用
