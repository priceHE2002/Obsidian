...
> dy 为连续函数，|φ(x)| ⩽ 设 k（k 为常数），求微分方程 满足初始条件 的特解 φ(x) +y 
> = φ(x) y(0) = 0 dx |y(x)|⩽k(1−e−x)。 x⩾0 y(x)，并证明当 时，有
>
> 题源：26 版 1000 第十五章基础 10　|　题源类型：1000　|　页码：p.306　|　模块：4.1 4.1
