...
> y′ (−∞,+∞) ̸=0，x=x(y) 设函数 在 内具有二阶导数，且 是 的反函数。 y =y(x) 
> y =y(x) 3 d2x dx （1）试将 所满足的微分方程 变换为 满足的微分方程； x=x(y) +(
> y+sinx) =0 y =y(x) dy2 dy 3 y(0)=0，y′(0)= （2）求变换后的微分方程
> 满足初始条件 的解。 2
>
> 题源：2003 数一二　|　题源类型：真题　|　页码：p.296　|　模块：2.2 二阶
