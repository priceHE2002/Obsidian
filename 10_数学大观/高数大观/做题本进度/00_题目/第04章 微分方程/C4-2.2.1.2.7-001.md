...
> [!note]- 题干预览（自动提取，仅供参考）
> 若 u (x)cosx+u 1 u′(x)cosx+u′(x)sinx=0，[u′(x)]2+[u′(x)]2 1 若 (0)=1，u (I) u 1 若该方程的另一特解 (II) 二阶变系数（数一二） 2.2.2 ▶2.2.2.1 二阶可降解 ▶2.2.2.1.1 不显含
>
> 题源：900 第六章数二 B 类 33　|　页码：p.293　|　模块：2.2 二阶
