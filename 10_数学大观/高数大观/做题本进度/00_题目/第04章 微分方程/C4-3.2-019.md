...
> 2sinx+cosx sinx+2cosx y′′+p(x)y′+q(x)y 设 ，y 是微分方程 的两个解
> ，则（ ） y (x)= (x)= =0 1 2 x x 2 2 ，q(x)=1 p(x)=1，q(x)= 
> A. p(x)= B. x x 1 1 ，q(x)=2 p(x)=2，q(x)= C. p(x)= D. x
>  x
>
> 题源：900 第六章数二 C 类 1　|　题源类型：900　|　页码：p.303　|　模块：3.2 二阶
