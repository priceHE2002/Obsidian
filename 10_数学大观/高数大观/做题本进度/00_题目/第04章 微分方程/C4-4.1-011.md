...
> dy 设P(x),Q(x)在(−∞,+∞)内连续，以T 为周期，函数y =y(x)是 =Q(x)的解，则y(
> 0)=y(T) +P(x)y dx 是 以 为周期的 y =y(x) T 必要非充分条件 充分非必要条件 A
> . B. 充要条件 既非充分又非必要条件 C. D.
>
> 题源：26 版 660 数二第 606 题　|　题源类型：660　|　页码：p.306　|　模块：4.1 4.1
