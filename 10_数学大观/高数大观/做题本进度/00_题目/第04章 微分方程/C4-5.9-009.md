...
> ⩾0) 有一平底容器，其内侧壁是由曲线 绕 轴旋转而成的旋转曲面（如图），容器的底面圆的半 x=φ(y)(y
>  y 3m3/min πm2/min 径为 2m。根据设计要求，当以 的速率向容器内注入液体时，液面的面积将
> 以 的速率均 匀扩大（假设注入液体前，容器内无液体）。 （1）根据 时刻液面的面积，写出 与 之间的关系式；
>  t t φ(y) （2）求曲线 的方程。（注：m 表示长度单位米，min 表示时间单位分。） x=φ(y)
>
> 题源：2003 数二　|　题源类型：真题　|　页码：p.329　|　模块：5.9 物理及实际应用
