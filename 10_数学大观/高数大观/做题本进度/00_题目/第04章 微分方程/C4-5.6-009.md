...
> f′(x) ⩾ 设函数 可导，且 0。曲线 经过坐标原点 O，其上任意一点 处的切线与 f(x) > y =
>  f(x)(x 0) M x △MTP 轴交于 T，又 垂直 轴于点 P。已知由曲线 =f(x)，直线 以及
>  轴所围图形的面积与 MP x y MP x 的面积之比恒为 3:2，求满足上述条件的曲线的方程。
>
> 题源：2020 数二　|　题源类型：真题　|　页码：p.324　|　模块：5.6 定积分应用（面积体积）
