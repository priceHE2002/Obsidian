...
> 2x+1 x+1 y′′− y′+ ex 已知 是方程 的解. 若非零函数 是该方程的一个解, 则下列说法中
> , 正确 y =0 u(x) x x 的是 ( ) x2u(x) x2u(x) 一定是该方程的解. 一定不是
> 该方程的解. A. B. exu(x) exu(x) 一定是该方程的解. 一定不是该方程的解. C. D. 
> 解微分方程 2 一阶
>
> 题源：900 第六章数二 B 类 4　|　题源类型：900　|　页码：p.279　|　模块：1.2 二阶及以上
