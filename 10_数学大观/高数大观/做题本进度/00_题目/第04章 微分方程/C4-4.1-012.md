...
> e−x f′′(x)+f′(x)= [0,+∞) 设定义在 上的二阶可导函数 满足 ，则下列结论中，错误的是
> （ ） f(x) 1+x2 f′(x) 有界. 有界. A. B. f(x) ′ 一定存在. 不一定存在. 
> C. lim f (x) D. lim f(x) x→+∞ x→+∞ ·第 页，共 页· 295 402
>
> 题源：900 第六章数二 B 类 8　|　题源类型：900　|　页码：p.306　|　模块：4.1 4.1
