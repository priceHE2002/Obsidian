...
> 设曲线 f(x)，其中 是可导函数，且 0。记曲线 与直线 0，x 及 40 y = f(x) f(x) >
>  y = f(x) y = = 1 所围成的曲边梯形绕 轴旋转一周所得的立体体积为 ，绕 轴旋转一周所得的立
> 体体积为 x = t(t > 1) x V y 1 。已知 是该曲边梯形面积值 的 倍，求该曲线的方程。 V
>  V +V S 3πt 2 1 2
>
> 题源：900 第六章数二 A 类 40　|　题源类型：900　|　页码：p.324　|　模块：5.6 定积分应用（面积体积）
