...
> 1 y′′−y′+ 微分方程 的通解为 y =0 y = 4 =0的解，且在x=0处y(x)取得极值3，则y
> (x)= y′′ 4y′ − 的解，且 在 处取得极值 4，则 + 3y = 0 y(x) x = 0 y(
> x) = y = f′′(x)−2f(x)=2e2x，则 及 f(x)= 14 = =−exsin2x B.
>  y =ex(sin2x−cos2x) D. y 13 y′′+2y′+y x−2y+5=0，且该曲线满足微
> 分方程 =0， 9 e−x B. y = 4+ x 2 =2(x+2)e−x D. y y+1=x−π 且在
> 该点和直线 相切的曲线为 B. y =cos3x+C sin3x 2 1 =cos3x− D. y sin3
> x 3 x→0 时，y(x)∼x，则 的解，当 =0 y(x)= ·第 页，共 页· 277 402
>
> 题源：2013 数三　|　题源类型：真题　|　页码：p.288　|　模块：2.2 二阶
