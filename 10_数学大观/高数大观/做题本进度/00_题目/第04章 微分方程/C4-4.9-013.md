...
> f′′(x)+af′(x)+f(x)=0，f(0)=f′(0)=1，则下列结论中，正确的个数是（） |a|⩾
> 2，函数 设 满足 f(x) Z +∞ ⃝ f(−x)dx 若 a>0，则 必收敛。 1 Z0 +∞ ⃝ 若
>  a>0，则 必收敛。 f(x)dx 2 Z0 +∞ ⃝ f(−x)dx 若 a<0，则 必收敛。 3 Z0
>  +∞ ⃝ 若 a<0，则 必收敛。 f(x)dx 4 0 A. 1 B. 2 C. 3 D. 4
>
> 题源：900 第六章数二 B 类 7　|　题源类型：900　|　页码：p.312　|　模块：4.9 微分方程解的反常积分
