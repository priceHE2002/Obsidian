...
> (−∞,+∞) F(x)=f(x)g(x)，其中 内满足以下条件： 设函数 在 f(x),g(x) ′ ′ 
> f (x)=4g(x), g (x)=f(x) f(0)=0，f(x)+2g(x)=e2x。 且 求 所满足
> 的一阶微分方程； (I) F(x) 求 的表达式。 (II) F(x)
>
> 题源：900 第六章数二 A 类 35　|　题源类型：900　|　页码：p.313　|　模块：5.2 含导数等式
