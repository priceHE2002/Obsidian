...
> (1+x2)f′(x)+2nxf(x)=0（n (−∞,+∞) 【15】设 在 内有连续导数，f(x)>0，
> 且满足 为大于 的 f(x) 1 正整数），f(0)=1， （I）求 f(x)； n S n （II）记 为
> 曲线 与 轴之间的无界区域的面积，求 S y =f(x) x lim n S n→∞ n−1
>
> 题源：880 第六章综合解答 15　|　题源类型：880　|　页码：p.312　|　模块：4.9 微分方程解的反常积分
