...
> f′′(x)+f′(x)=e−xsin3x，则下列结论中，错误的是（ [0,+∞) 设定义在 上的二阶可导函
> 数 满足 ） f(x) 有界 存在 A. f(x) B. lim f(x) x→+∞ f′(0)=0，则 若
>  是单调函数 是周期函数 C. f(x) D. f(x)
>
> 题源：900 第六章数二 B 类 9　|　题源类型：900　|　页码：p.312　|　模块：4.9 微分方程解的反常积分
