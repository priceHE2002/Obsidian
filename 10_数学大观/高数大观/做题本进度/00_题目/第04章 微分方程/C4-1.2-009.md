...
> y′′ −y′ 设 和 是微分方程 的两个特解，则该方程的通解能表示成 y (x) y (x) +y = 0
>  C y (x)+C y (x) 1 2 1 1 2 2 （C 是任意常数）的充分条件为（ ） ,C 1 2
>  (x)y′(x)−y′(x)y (x)y′(x)−y′(x)y (x)̸=0 A. y B. y (x)=
> 0 1 2 1 2 2 1 2 1 (x)y′(x)+y′(x)y (x)y′(x)+y′(x)y (x)̸
> =0 C. y D. y (x)=0 1 2 1 2 2 1 2 1
>
> 题源：900 第六章数二 A 类 6　|　题源类型：900　|　页码：p.279　|　模块：1.2 二阶及以上
