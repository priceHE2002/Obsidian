...
> =f(x)(0≤x<+∞) 0≤f(x)≤ex−1； 假设：（1）函数 满足条件 和 y f(0)=0 =e
> x−1 （2）平行于 轴的动直线 与曲线 和 分别相交于点 和 ； y MN y =f(x) y P P 1
>  2 （3）曲线 =f(x)，直线 与 轴所围封闭图形的面积 恒等于线段 的长度。 y MN x S P P
>  1 2 求函数 的表达式。 y =f(x)
>
> 题源：1993 数三　|　题源类型：真题　|　页码：p.323　|　模块：5.6 定积分应用（面积体积）
