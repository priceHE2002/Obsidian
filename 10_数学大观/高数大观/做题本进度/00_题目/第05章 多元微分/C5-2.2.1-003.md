...
> 设 在点 的某邻域内有定义，f(0,0)=0，且 f(x,y) (0,0) f(x,y) p 为常数) li
> m =1+k (k x→0 x2+y2 y→0 证明：f(x,y) 在点 处连续； (I) (0,0) ̸=
> −1 证明：当 时，f(x,y) 在点 处不可微； (II) k (0,0) =−1 证明：当 时，f(x,
> y) 在点 处可微。 (III) k (0,0) ·第 页，共 页· 328 402
>
> 题源：880 第四章拓展解答　|　题源类型：880　|　页码：p.339　|　模块：2.2 已知重极限推导其他
