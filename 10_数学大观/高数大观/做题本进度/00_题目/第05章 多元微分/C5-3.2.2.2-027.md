...
备注:
---
# 3.2.2.2-027 · 多元复合函数求偏导

> [!question] 题目（程序自动提取，公式可能有偏差）
> f(x,y)=−f(y,x)，则下列结论中，错误的是（ 若二元函数 存在二阶连续偏导数，且满足 ） f(x,
> y) f′′(x,y)=f′′(x,y) f′′(x,y)=−f′′(y,x) A. B. 11 22 11
>  22 f′′(x,y)=f′′(x,y) f′′(x,y)=−f′′(y,x) C. D. 12 21 1
> 2 21 ▶3.2.2.3 套 f f
>
> 题源：900 第四章数二 A 类 9　|　题源类型：900　|　页码：p.353　|　模块：3.2 多元复合函数求偏导
