...
> f(x,y)=|x|+y|y|，则（）. 设函数 f′(0,0) 存在，f′(0,0) f′(0,0) 存在
> ，f′(0,0) 存在 不存在 A. B. x y x y f′(0,0) 不存在，f′(0,0) f′(0
> ,0) 不存在，f′(0,0) 存在 不存在 C. D. x y x y
>
> 题源：26 版 1000 第十三章强化 1　|　题源类型：1000　|　页码：p.343　|　模块：3.1 多元函数求偏导
