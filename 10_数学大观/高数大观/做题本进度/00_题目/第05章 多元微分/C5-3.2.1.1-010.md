...
备注:
---
# 3.2.1.1-010 · 多元复合函数求偏导

> [!question] 题目（程序自动提取，公式可能有偏差）
> y ∂z ∂z y，则下列关于函数 2xyexy+ 已知函数 可导，二元函数 满足 f(u) z(x,y) 
> = f(xy)f x +y = f(u) x x ∂x ∂y 的说法中，正确的是（ ） 曲线 是凸曲线。 曲
> 线 是凹曲线。 A. y =f(u) B. y =f(u) 曲线 存在拐点。 曲线 存在渐近线。 C. y 
> =f(u) D. y =f(u)
>
> 题源：900 第四章数二 B 类 10　|　题源类型：900　|　页码：p.348　|　模块：3.2 多元复合函数求偏导
