...
> f(x,y)−Ax−By−C p [例 设 在点 处连续，且 =0。证明： 5.3] f(x,y) (x ,
> y ) lim 0 0 → (x−x )2+(y−y x x )2 0 → 0 0 y y0 （I）f(x 
> +C； ,y )=Ax +By 0 0 0 0 )=A，f′(x （II）f′(x )=B； ,y ,y 0
>  0 0 0 y x （III）f(x,y) 在点 处可微，且 =Adx+Bdy； (x ,y ) df 0
>  0 (x0,y0) （IV）（数学一要求）曲面 在点 处的切平面为 =Ax+By+C。 z =f(x,y)
>  (x ,y ) z 0 0
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.339　|　模块：2.2 已知重极限推导其他
