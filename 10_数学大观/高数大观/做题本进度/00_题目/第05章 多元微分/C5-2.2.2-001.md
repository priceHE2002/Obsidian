...
> f(x,y)−xy 已知函数 在点 的某个邻域内连续，且 =1，则 (3) f(x,y) (0,0) lim
>  ( ) (x2+y2)2 x→0 y→0 点 不是 的极值点。 A. (0,0) f(x,y) 点 是 的
> 极大值点。 B. (0,0) f(x,y) 点 是 的极小值点。 C. (0,0) f(x,y) 根据所给条
> 件无法判别点 是否为 的极值点。 D. (0,0) f(x,y)
>
> 题源：2003 数一　|　题源类型：真题　|　页码：p.341　|　模块：2.2 已知重极限推导其他
