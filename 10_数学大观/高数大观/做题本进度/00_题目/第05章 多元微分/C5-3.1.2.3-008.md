...
> Z Z x+y y f(x+y−t)dt，v(x,y)=yf(x+y)+ 设函数 f(x+t)dt，其中函数
>  具有 u(x,y)=f(xy)+ f(t) y−x 0 一阶导数，则下列结论中，正确的是（ ） ∂u ∂u
>  ∂u ∂v 若 x=y，则 若 x=y，则 A. = B. = ∂x ∂y ∂x ∂x ∂u ∂u ∂u 
> ∂v x̸=y，则 ̸= x̸=y，则 ̸= 若 若 C. D. ∂x ∂y ∂x ∂x
>
> 题源：900 第四章数二 A 类 10　|　题源类型：900　|　页码：p.347　|　模块：3.1 多元函数求偏导
