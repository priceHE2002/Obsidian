...
> ),f′(x f′(x 【2】设 均存在，则下列选项正确的是（ ）。 ,y ,y ) 0 0 0 0 x y
>  存在 在 处连续 A. lim f(x,y) B. f(x,y) (x ,y ) 0 0 → x x 0 
> → y y0 U˚(x 存在 在 内有定义 C. lim f(x,y ) D. f(x,y) ,y ) 0 
> 0 0 x→x0
>
> 题源：880 第四章基础选择 2　|　题源类型：880　|　页码：p.333　|　模块：1.1 基本概念题
