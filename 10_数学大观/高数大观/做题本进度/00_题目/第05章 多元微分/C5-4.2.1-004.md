...
> f(x)>0，f′(0)=0，则函数 设函数 具有二阶连续导数，且 在点 处取得极小值 f(x) z =f(
> x)lnf(y) (0,0) 的一个充分条件是（） f(0)>1，f′′(0)>0 f(0)>1，f′′(0
> )<0 A. B. f(0)<1，f′′(0)>0 f(0)<1，f′′(0)<0 C. D.
>
> 题源：2011 数一　|　题源类型：真题　|　页码：p.369　|　模块：4.2 无条件极值
