...
> −3， 【15】设函数 的全微分为 (2ax+by)dx+(2by+ax)dy（a,b 为常数），且 f(x
> ,y) df(x,y) = f(0,0) = f′(1,1)=3。试求： x （I）f(x,y)； (−1,
> −1) （II）点 到曲线 上的点的距离的最大值。 f(x,y)=0
>
> 题源：880 第四章综合解答 15　|　题源类型：880　|　页码：p.378　|　模块：4.3 条件极值
