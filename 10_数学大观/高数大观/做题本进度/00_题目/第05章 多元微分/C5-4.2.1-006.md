...
> [!note]- 题干预览（自动提取，仅供参考）
> 已知函数f(x,y)具有二阶连续偏导数，函数g(x)具有二阶导数且g(0)=0。对于函数z(x,y)=f(g(x),g(y))， 下列说法中，正确的是（ 若 A. x=0 若点 B. (0,0) 若点 C. (0,0) 极值点。 若点 D. (0,0) 极值点。
>
> 题源：900 第四章数二 B 类 11　|　页码：p.369　|　模块：4.2 无条件极值
