...
> 8 ><x4−y4 ̸=0, x2+y2 , x2+y2 设 则 在点 处 f(x,y)= f(x,y) (
> 0,0) >: x2+y2 0, =0, f′(0,0) f′(0,0) 连续，但偏导数 和 不存在。 A.
>  x y f′(0,0) f′(0,0) 连续且偏导数 和 都存在，但不可微。 B. x y f′ f′ 不
> 连续。 可微但 和 C. y x f′ f′ 连续。 和 可微且 D. y x
>
> 题源：26 版 660 数一二三第 243 题　|　题源类型：660　|　页码：p.336　|　模块：1.2 已知具体多元函数判断其他
