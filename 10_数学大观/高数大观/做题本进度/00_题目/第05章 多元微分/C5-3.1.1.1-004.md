...
> p 【1】设 x2+y4，则下列选项正确的是（ ）. f(x,y)=arcsin f′(0,0) 存在，f′
> (0,0) f′(0,0) 不存在，f′(0,0) 存在 存在 A. B. x y x y 存在，f′(0,
> 0) f′(0,0) 不存在，f′(0,0) f′(0,0) 不存在 不存在 D. C. y x y x
>
> 题源：880 第四章基础选择 1　|　题源类型：880　|　页码：p.343　|　模块：3.1 多元函数求偏导
