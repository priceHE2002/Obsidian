...
> f(x)>0，f′(0)=0，则函数 f(x)f(y) 设函数 具有二阶连续导数，且 在点 处取得极小 f(
> x) z(x,y)= (0,0) 值的一个充分条件是（） f(0)<1，f′′(0)<0 f(0)>1，f′
> ′(0)<0 A. B. f(0)<1，f′′(0)>0 f(0)>1，f′′(0)>0 C. D.
>
> 题源：900 第四章数二 A 类 17　|　题源类型：900　|　页码：p.369　|　模块：4.2 无条件极值
