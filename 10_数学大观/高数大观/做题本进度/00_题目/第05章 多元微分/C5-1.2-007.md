...
备注:
---
# 1.2-007 · 已知具体多元函数判断其他

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 p 1 < (x,y)̸=(0,0), x2+y2sin , x2+y2 设 则 在点 处 f(x,y)
> = f(x,y) (0,0) ( ). : 0, (x,y)=(0,0), 两个偏导数都存在，函数也连续 两
> 个偏导数都存在，但函数不连续 A. B. 偏导数不存在，但函数连续 偏导数不存在，函数也不连续 C. D.
>
> 题源：26 版 1000 第十三章强化 6　|　题源类型：1000　|　页码：p.335　|　模块：1.2 已知具体多元函数判断其他
