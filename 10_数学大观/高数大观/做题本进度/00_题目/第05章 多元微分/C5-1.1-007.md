...
> 函数 在 点可微的充分条件是 f(x,y) (0,0) ′ ′ ′ ′ 且 A. limf (x,0)=f 
> (0,0) limf (0,y)=f (0,0) x x y y x→0 y→0 [f(x,y)−f(0,0
> )]=0 B. lim (x,y)→(0,0) f(0,y)−f(0,0) f(x,0)−f(0,0) 和 
> 都存在 lim C. lim x y y→0 x→0 ′ ′ ′ ′ 且 D. lim f (x,y)=f 
> (0,0) lim f (x,y)=f (0,0) x x y y (x,y)→(0,0) (x,y)→(0
> ,0)
>
> 题源：26 版 660 数一二三第 244 题　|　题源类型：660　|　页码：p.333　|　模块：1.1 基本概念题
