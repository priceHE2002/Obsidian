...
> 8 xy2 ><p (x,y)̸=(0,0), , x2+y2 设函数 则下列命题中，正确的是（ ） f(x
> ,y)= >: 0, (x,y)=(0,0), f′′ f′′ f′′ f′′ 存在。 不存在，但 和 均不
> 存在。 (0,0) (0,0) (0,0) B. A. (0,0) yx xy yx xy f′′ f′′ 
> f′′ f′′ 和 均存在。 不存在。 存在，但 (0,0) D. (0,0) (0,0) (0,0) C.
>  yx xy yx xy
>
> 题源：900 第四章数二 B 类 3　|　题源类型：900　|　页码：p.345　|　模块：3.1 多元函数求偏导
