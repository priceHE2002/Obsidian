...
> f(x,y) 设函数 在点 处连续，且满足 则（ ） f(x,y) (1,1) lim =1, 9 (x,y
> )→(1,1)x3+y3− (x2+y2)+6(x+y)−5 2 点 是 的极大值点 点 是 的极小值点 A
> . (1,1) f(x,y) B. (1,1) f(x,y) 点 不是 的极值点 无法确定点 是否为 的极值
> 点 C. (1,1) f(x,y) D. (1,1) f(x,y)
>
> 题源：900 第四章数二 B 类 14　|　题源类型：900　|　页码：p.342　|　模块：2.2 已知重极限推导其他
