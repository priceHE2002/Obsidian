...
> =(2x−y2−2y)dx+(−2xy−2x+y3+3y)dy,且φ(0,0)=0。 设函数f(x,y)连续
> ，z =φ(x,y)的全微分为dz f(x,y) 若 =1，则（ ） lim (x,y)→(0,0)φ(x,
> y) 点 是 的极大值点。 点 是 的极小值点。 A. (0,0) f(x,y) B. (0,0) f(x,
> y) 点 不是 的极值点。 不能确定点 是否为 的极值点。 C. (0,0) f(x,y) D. (0,0)
>  f(x,y)
>
> 题源：900 第四章数二 B 类 15　|　题源类型：900　|　页码：p.342　|　模块：2.2 已知重极限推导其他
