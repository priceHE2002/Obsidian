...
> f(x,y) 设函数 满足 =1，α>0 且 f(0,0)=0，则下列结论中，错误的是（ ） f(x,y) 
> lim (x,y)→(0,0)[sin(|x|+|y|)]α 在点 处连续. A. f(x,y) (0,0)
>  在点 处取得极小值. B. f(x,y) (0,0) f′(0,0) f′(0,0) 若 α=1，则 和 
> 都不存在. C. x y f′(0,0) f′(0,0) 若 α>1，则 和 都存在且均不为 D. 0. x
>  y 偏导数 3 多元函数求偏导
>
> 题源：900 第四章数二 A 类 5　|　题源类型：900　|　页码：p.342　|　模块：2.2 已知重极限推导其他
