...
备注:
---
# 1.2-005 · 已知具体多元函数判断其他

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 >><cosxy, ̸=0, xy 设函数 则下列结论中，正确的是（ ） f(x,y)= cosy, x
> =0, >>: cosx, y =0, ∂2f 函数 在点 处连续，且 存在。 A. f(x,y) (0,0
> ) ∂x∂y ∂2f 函数 在点 处连续，但 不存在。 B. f(x,y) (0,0) ∂x∂y ∂2f 函
> 数 在点 处不连续，但 存在。 C. f(x,y) (0,0) ∂x∂y ∂2f 函数 在点 处不连续，且 
> 不存在。 D. f(x,y) (0,0) ∂x∂y
>
> 题源：900 第四章数二 B 类 2　|　题源类型：900　|　页码：p.334　|　模块：1.2 已知具体多元函数判断其他
