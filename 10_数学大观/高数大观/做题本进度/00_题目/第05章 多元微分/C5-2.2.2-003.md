...
> f(x,y) 设函数 连续，且 =1，则（ ） f(x,y) lim (x,y)→(0,0)x4+y3−2x
> 2−y2 点 不是 的极值点. 点 是 的极大值点. A. (0,0) f(x,y) B. (0,0) f(
> x,y) 点 是 的极小值点. 不能确定点 是否为 的极值点. C. (0,0) f(x,y) D. (0,
> 0) f(x,y)
>
> 题源：900 第四章数二 B 类 12　|　题源类型：900　|　页码：p.341　|　模块：2.2 已知重极限推导其他
