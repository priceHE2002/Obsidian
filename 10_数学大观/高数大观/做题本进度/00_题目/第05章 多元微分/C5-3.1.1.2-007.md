...
> √ √ ∂z ∂z 设 y)，则 z =ln( x+ x +y = ∂x ∂y . ∂x ∂2f 确定，其中
> 函数g(y)可微，且g(y)̸=0，则 = ∂u∂v ∂f(cid:12) ∂f(cid:12) =x2−y
> 2，则 与 依次是（ ） ∂u ∂v u=1 u=1 v=1 v=1 1 1 − 0,− C. ,0 D. 
> 2 2 8 f′(x,y)= . x 2y ∂f(cid:12) ∂f(cid:12) =4x2−y2，则 
> ， 依次是（ ） x ∂u ∂v u=1 u=1 v=0 v=0 1 1 1 − − C. , 2 D. 2
> , 2 2 2 ·第 页，共 页· 333 402
>
> 题源：26 版 660 数一二三第 88 题　|　题源类型：660　|　页码：p.344　|　模块：3.1 多元函数求偏导
