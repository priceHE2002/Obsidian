...
备注:
---
# 1.2-010 · 已知具体多元函数判断其他

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 xy < (x,y)̸=(0,0), , x2+y2 二元函数 在点 处（ ） (1) f(x,y)= 
> (0,0) : 0, (x,y)=(0,0) 连续，偏导数存在。 连续，偏导数不存在。 A. B. 不连续，
> 偏导数存在。 不连续，偏导数不存在。 C. D.
>
> 题源：1997 数一　|　题源类型：真题　|　页码：p.335　|　模块：1.2 已知具体多元函数判断其他
