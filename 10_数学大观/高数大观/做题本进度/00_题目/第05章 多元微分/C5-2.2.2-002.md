...
> f(x,y)+x+y−1 p g(x,y)=f(1+xy,x2+y2)，证 【例 5.10】设 有二阶连续偏
> 导数，且 =0。若 f(x,y) lim (x−1)2+y2 x→1 y→0 明：g(x,y) 在点 处取得
> 极值，并判断是极大值还是极小值。 (0,0)
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.341　|　模块：2.2 已知重极限推导其他
