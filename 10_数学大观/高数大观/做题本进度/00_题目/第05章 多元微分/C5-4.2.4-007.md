...
> )=0，F′(x )=0，F′(x 【6】设F(x,y)在点(x )的某邻域内有二阶连续偏导数，且F(x )
> >0， ,y ,y ,y ,y 0 0 0 0 0 0 0 0 x y F′′ )<0，则由方程 确定的隐函
> 数 在 处（ ）。 (x ,y F(x,y)=0 y =y(x) x=x 0 0 0 xx 取得极小值 取得
> 极大值 A. B. 不取得极值 不能确定是否取得极值 C. D. 已知极值反求参数
>
> 题源：880 第四章综合选择 6　|　题源类型：880　|　页码：p.375　|　模块：4.2 无条件极值
