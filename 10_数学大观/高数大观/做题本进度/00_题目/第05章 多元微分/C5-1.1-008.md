...
> 下列（ ）选项条件成立时，能够推出函数 在点 处可微，且全微分 =0。 f(x,y) (x ,y ) df(
> x,y) 0 0 (x0,y0) )=f′(x f′(x A. ,y ,y )=0 0 0 0 0 x y 
> ∆x∆y p 在点 处的全增量 B. f(x,y) (x ,y ) ∆f = 0 0 (∆x)2+(∆y)2
>  (∆x)2+(∆y)2 sin p 在点 处的全增量 C. f(x,y) (x ,y ) ∆f = 0 0
>  (∆x)2+(∆y)2 1 (∆x)2+(∆y)2 在点 处的全增量 D. f(x,y) (x ,y ) 
> ∆f = sin 0 0 (∆x)2+(∆y)2 已知具体多元函数判断其他
>
> 题源：880 第四章拓展选择　|　题源类型：880　|　页码：p.333　|　模块：1.1 基本概念题
