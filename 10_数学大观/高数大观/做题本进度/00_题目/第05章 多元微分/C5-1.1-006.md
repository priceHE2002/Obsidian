...
> 二元函数 在点 处可微的一个充分条件是（ ）. (7) f(x,y) (0,0) [f(x,y)−f(0,0
> )]=0 A. lim (x,y)→(0,0) f(x,0)−f(0,0) f(0,y)−f(0,0) =0
> ，且 B. lim lim =0 x y x→0 y→0 f(x,y)−f(0,0) p C. lim =0
>  (x,y)→(0,0) x2+y2 ′ ′ ′ ′ (x,0)−f (0,y)−f (0,0)]=0，且 
> D. lim[f lim[f (0,0)]=0 x x y y x→0 y→0
>
> 题源：2007 数二　|　题源类型：真题　|　页码：p.333　|　模块：1.1 基本概念题
