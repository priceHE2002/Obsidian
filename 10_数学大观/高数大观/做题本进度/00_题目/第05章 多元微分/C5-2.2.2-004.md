...
> f(x,y)−xy 1 设函数 在点 处连续且 ，则（ ） = f(x,y) (0,0) lim x2+y2
>  2 (x,y)→(0,0) 点 是 的极大值点. A. (0,0) f(x,y) 点 是 的极小值点. B
> . (0,0) f(x,y) 点 不是 的极值点. C. (0,0) f(x,y) 根据已知条件无法判断点 
> 是否为 的极值点. D. (0,0) f(x,y) ·第 页，共 页· 330 402
>
> 题源：900 第四章数二 B 类 13　|　题源类型：900　|　页码：p.341　|　模块：2.2 已知重极限推导其他
