...
> 设可微函数 在点 取得极小值，则下列结论正确的是（ ） f(x,y) (x ,y ) 0 0 在 处的导数等
> 于零. 在 处的导数大于零. A. f(x ,y) y =y B. f(x ,y) y =y 0 0 0 0
>  在 处的导数小于零. 在 处的导数不存在. C. f(x ,y) y =y D. f(x ,y) y =y
>  0 0 0 0
>
> 题源：2003 数三　|　题源类型：真题　|　页码：p.368　|　模块：4.2 无条件极值
