...
备注:
---
# 3.4.1.1.3-001 · 多元隐函数求偏导

> [!question] 题目（程序自动提取，公式可能有偏差）
> xy−zlny+exz 设有三元方程 =1，根据隐函数存在定理，存在点 的一个邻域，在此邻域内该 (10) 
> (0,1,1) 方程（ ） 只能确定一个具有连续偏导数的隐函数 =z(x,y)。 A. z 可确定两个具有连
> 续偏导数的隐函数 和 =z(x,y)。 B. y =y(x,z) z 可确定两个具有连续偏导数的隐函数 和 
> =z(x,y)。 C. x=x(y,z) z 可确定两个具有连续偏导数的隐函数 和 =y(x,z)。 D. 
> x=x(y,z) y
>
> 题源：2005 数一; 880 第四章基础选择 5　|　题源类型：真题　|　页码：p.358　|　模块：3.4 多元隐函数求偏导
