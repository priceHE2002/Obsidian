...
备注:
---
# 2.4-009 · 利用含二重积分等式求函数表达式

> [!question] 题目（程序自动提取，公式可能有偏差）
> Z ZZ x2 −t)dt+ f(x2 x3 【47】设函数 满足 f(x+y)dσ+a，其中 是由 与 1
> ， f(x) f(x) = x D y = y = 0 D =−1 及 轴所围平面有界闭区域，f(1)=0，
> 且 在 上的平均值为 3，求常数 的值。 y y f(x) [0,1] a 抽象函数二重积分
>
> 题源：25 版 0.1w 强化 47　|　题源类型：150/0.1w　|　页码：p.410　|　模块：2.4 利用含二重积分等式求函数表达式
