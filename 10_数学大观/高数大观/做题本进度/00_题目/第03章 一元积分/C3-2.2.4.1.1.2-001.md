...
备注:
---
# 2.2.4.1.1.2-001 · 物理应用（数一二）

> [!question] 题目（程序自动提取，公式可能有偏差）
> 1 ⩾ x2 +y2 x2 +y2 一容器的内侧是由右图中曲线绕 轴旋转一周而成的曲面，该曲线由 与 y =
>  2y y = 2 1 ⩽ 连接而成。 1 y 2 （I）求容器的容积； gm/s2， （II）若将容器内盛
> 满的水从容器顶部全部抽出，至少需要做多少功？（长度单位为 m，重力加速度为 103kg/m3。） 水的密度为
>  ·第 页，共 页· 255 402
>
> 题源：2011 数二　|　题源类型：真题　|　页码：p.266　|　模块：2.2 物理应用（数一二）
