...
> [!note]- 题干预览（自动提取，仅供参考）
> 某建筑工地打地基时，需用汽锤将桩打进土层。汽锤每次击打，都将克服土层对桩的阻力而做功。设土层对 桩的阻力的大小与桩被打进地下的深度成正比（比例系数为 根据设计方案，要求汽锤每次击打桩时所做的功相等。问： （I）汽锤第三次击打后，可将桩打进地下多深？ （II）记击打 次后，桩被击打进土层的深度为 n （注：m 表示长度单位米。）
>
> 题源：900 第三章数二 B 类 65　|　页码：p.269　|　模块：2.2 物理应用（数一二）
