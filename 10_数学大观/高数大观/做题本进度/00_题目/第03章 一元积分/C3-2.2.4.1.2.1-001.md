...
备注:
---
# 2.2.4.1.2.1-001 · 物理应用（数一二）

> [!question] 题目（程序自动提取，公式可能有偏差）
> 为清除井底的污泥，用缆绳将抓斗放入井底，抓起污泥后提出井口（如图）。已知井深 30m，抓斗自重 400N， 
> 缆绳每米重 50N，抓斗抓起的污泥重 2000N，提升速度为 3m/s。在提升过程中，污泥以 的速率从抓 2
> 0N/s 斗缝隙中漏掉。现将抓起污泥的抓斗提升至井口，问克服重力需作多少焦耳的功？（说明：⃝ 1N×1m=1
> J； 1 分别表示米，牛顿，秒，焦耳；⃝ 抓斗的高度及位于井口上方的缆绳长度忽略不计。） m,N,s,J 2
>  ·第 页，共 页· 257 402
>
> 题源：1999 数一二　|　题源类型：真题　|　页码：p.268　|　模块：2.2 物理应用（数一二）
