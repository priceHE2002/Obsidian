...
备注:
---
# 2.1.3.1.1.3-014 · 几何应用

> [!question] 题目（程序自动提取，公式可能有偏差）
> 由相交于三点 )（其中 ）的两曲线 所围 373 (x ,y ), (x ,y ), (x ,y x <x 
> <x y =f(x)>0, y =g(x)>0 1 1 2 2 3 3 1 2 3 成的图形绕 轴旋转一周所
> 得旋转体体积为 x Z Z x3 x3 π[f(x)−g(x)]2dx π[f2(x)−g2(x)]dx A
> . B. Zx1 Zx 1 x3 x 3 π[f2(x)−g2(x)]dx π|f2(x)−g2(x)|dx
>  D. C. x1 x1
>
> 题源：660 数二 373　|　题源类型：660　|　页码：p.250　|　模块：2.1 几何应用
