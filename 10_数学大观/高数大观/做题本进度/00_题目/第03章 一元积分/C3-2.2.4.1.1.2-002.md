...
备注:
---
# 2.2.4.1.1.2-002 · 物理应用（数一二）

> [!question] 题目（程序自动提取，公式可能有偏差）
> 4 − ⩾ x2 1)2 一容器的内侧是由图中曲线绕 轴旋转一周而成的曲面，该曲线由 与 64 y + (y
>  = 1 (y 1) 27 x2 y2 ⩽1) 连接而成. + =1 (y 9 4 （I）求容器的容积； （
> II）如图所示，现容器中的液面位于 轴下方，若将容器中的水从容器顶部全部抽出，顶部出口在图中 x (0,2)
>  处，至少需要做多少功？ m/s2，水的密度为 103 kg/m3） （长度单位：m，重力加速度为 g
>
> 题源：900 第三章数二 B 类 64　|　题源类型：900　|　页码：p.267　|　模块：2.2 物理应用（数一二）
