...
> x ，x∈[0,1]，定义函数列： 设函数 f(x)= 1+x ··· ··· f (x)=f(x), f 
> (x)=f f (x) , , f (x)=f f (x) , n−1 1 2 1 n 记 是由曲线 (x)
> ，直线 及 轴所围平面图形的面积，求极限 S y =f x=1 x lim nS n n n n→∞ ·第 
> 页，共 页· 231 402
>
> 题源：2014 数二　|　题源类型：真题　|　页码：p.242　|　模块：2.1 几何应用
