...
备注:
---
# 2.2.3.2-007 · 物理应用（数一二）

> [!question] 题目（程序自动提取，公式可能有偏差）
> 半圆形闸门半径为 R(米), 将其垂直放入水中，且直径与水面齐. 设 若坐标原点取在圆心，x 轴正向 ρg 
> =1. 朝下，则闸门所受压力 为 p Z Z p p R R R2−x2dx R2−x2dx A. B. 2
>  Z0 Z0 p p R R R2−x2dx 2(R−x) R2−x2dx C. 2x D. 0 0
>
> 题源：660 数二 377　|　题源类型：660　|　页码：p.265　|　模块：2.2 物理应用（数一二）
