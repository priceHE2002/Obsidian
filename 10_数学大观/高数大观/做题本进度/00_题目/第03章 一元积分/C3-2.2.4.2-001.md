...
> [!note]- 题干预览（自动提取，仅供参考）
> 某建筑工程打地基时，需用汽锤将桩打进土层。汽锤每次击打，都将克服土层对桩的阻力而做功。设土层对桩 的阻力的大小与桩被打进地下的深度成正比（比例系数为 根据设计方案，要求汽锤每次击打桩时所作的功与前一次击打时所作的功之比为常数 （I）汽锤击打桩 （II）若击打次数不限，汽锤至多能将桩打进地下多深？（注：m
>
> 题源：2003 数一　|　页码：p.269　|　模块：2.2 物理应用（数一二）
