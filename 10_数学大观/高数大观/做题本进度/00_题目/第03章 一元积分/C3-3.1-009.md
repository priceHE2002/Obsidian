...
> [0,+∞) 设非负函数 在 上连续，给出以下三个命题： f(x) Z Z +∞ +∞ ⃝ f2(x)dx 
> 若 收敛，则 收敛. f(x)dx 1 Z 0 0 +∞ ⃝ xpf(x) 若存在 p>1，使得 存在，则 
> 收敛. lim f(x)dx 2 x→+∞ 0 ·第 页，共 页· 261 402
>
> 题源：2024 数二　|　题源类型：真题　|　页码：p.272　|　模块：3.1 审敛
