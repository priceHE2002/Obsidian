...
备注:
---
# 2.2.4.1.1.2-006 · 物理应用（数一二）

> [!question] 题目（程序自动提取，公式可能有偏差）
> 有一内表面为旋转抛物面的水缸，其深为 a（单位：米），缸口直径为 2a（单位：米），缸内盛满了水，设水 的密
> 度为 ρ（单位：千克/立方米）. 若以每秒 立方米的速率将缸中的水全部抽出，问： Q (1) 需做多少功？ 
> (2)
>
> 题源：1000 第十二章基础 4　|　题源类型：1000　|　页码：p.268　|　模块：2.2 物理应用（数一二）
