...
备注:
---
# 2.1.3.1.1.4-004 · 几何应用

> [!question] 题目（程序自动提取，公式可能有偏差）
> [0,+∞) 设 是定义在 上的非负函数，且对于任意的 a>0，x=0，x=a，y 与 轴所围成 y =f(
> x) =f(x) x =x3 的平面图形绕 轴旋转一周与绕 轴旋转一周所形成的旋转体体积相同，则 与 所围成
> 的平 x y y =f(x) y 面图形面积为
>
> 题源：900 第三章数二 A 类 69　|　题源类型：900　|　页码：p.251　|　模块：2.1 几何应用
