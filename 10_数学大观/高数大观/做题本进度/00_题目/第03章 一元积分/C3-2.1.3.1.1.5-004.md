...
备注:
---
# 2.1.3.1.1.5-004 · 几何应用

> [!question] 题目（程序自动提取，公式可能有偏差）
> 3a xf′(x) x2（a 设函数 在闭区间 上连续，在开区间 内大于零，并满足 为常数）， f(x) [
> 0,1] (0,1) = f(x)+ 2 又曲线 与 所围的图形 的面积值为 2，求函数 =f(x)，并问 
> 为何值时，图形 y =f(x) x=1,y =0 S y a S 绕 轴旋转一周所得的旋转体的体积最小。 x
>
> 题源：1997 数二　|　题源类型：真题　|　页码：p.251　|　模块：2.1 几何应用
