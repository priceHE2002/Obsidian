...
备注:
---
# 2.1.3.1.2.4-003 · 几何应用

> [!question] 题目（程序自动提取，公式可能有偏差）
> p x−2 g(x) f′(x) f(x)，g′(x) 已知函数 分别满足 ，f(0) 1，g(1) 0。求
> 曲线 f(x),g(x) = 2 = + = = x−2 x x=−1 所围图形绕直线 旋转一周所成旋转体体
> 积。 f(x)+g(y)=0
>
> 题源：1000 第十章强化 19　|　题源类型：1000　|　页码：p.254　|　模块：2.1 几何应用
