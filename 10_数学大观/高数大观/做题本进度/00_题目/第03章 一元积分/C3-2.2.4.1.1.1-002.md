...
备注:
---
# 2.2.4.1.1.1-002 · 物理应用（数一二）

> [!question] 题目（程序自动提取，公式可能有偏差）
> x2+y2 在一个高为 的圆柱形容器内储存某种液体，并将容器横放．底面圆的方程为 =1（单位：m）．如 1m
>  0.2m3/min 果容器内存满了液体后，以 的速率将液体从容器顶端抽出． （1）当液面在 时，求液面下降
> 的速率； y =0 1m3 （2）如果 液体所受重力为 1N，求抽完全部液体需做多少功？ ▶2.2.4.1.
> 1.2 旋转体
>
> 题源：1000 第十二章基础 5　|　题源类型：1000　|　页码：p.266　|　模块：2.2 物理应用（数一二）
