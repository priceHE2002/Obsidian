...
备注:
---
# 2.1.3.1.1.6-001 · 几何应用

> [!question] 题目（程序自动提取，公式可能有偏差）
> =e−x(x≥0)。 设曲线方程 y e−x，x （1）把曲线 轴，y 轴和直线 所围成平面图形绕 轴旋转一
> 周，得一旋转体，求此 y = x = ξ(ξ > 0) x 1 的 a。 旋转体体积 V(ξ)；求满足 li
> m V(ξ) V(a)= 2ξ→+∞ （2）在此曲线上找一点，使过该点的切线与两个坐标轴所夹平面图形的面积最
> 大，并求出该面积。
>
> 题源：1992 数三　|　题源类型：真题　|　页码：p.252　|　模块：2.1 几何应用
