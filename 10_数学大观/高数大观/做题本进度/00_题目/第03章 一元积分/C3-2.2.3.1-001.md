...
备注:
---
# 2.2.3.1-001 · 物理应用（数一二）

> [!question] 题目（程序自动提取，公式可能有偏差）
> 如图，x 轴上的一线密度为常数 µ，长度为 的细杆，若质量为 的质点到杆右端的距离为 a，已知引 (5) l
>  m 力系数为 k，则质点和细杆之间引力的大小为（ ） Z Z Z Z l 0 l 0 kmµdx kmµd
> x kmµdx kmµdx 2 A. B. C. 2 D. 2 (a−x)2 (a−x)2 (a+x)2 (
> a+x)2 −l −l 0 0 2
>
> 题源：1991 数二; 基础严选第六章选择 5　|　题源类型：真题　|　页码：p.262　|　模块：2.2 物理应用（数一二）
