...
> y′−p(x)y [0,+∞) f(0)⩾0，其中 [0,+∞) 【21】设在 上的可导函数 满足 >0，且
>  在 上为正 y =f(x) p(x) 值连续函数，当 时，下列选项中正确的是（ ）。 0<a<b A. f
> (0)<f(a)<f(b) B. f(b)<f(a)<f(0) C. f(b)<f(0)<f(a) D. f
> (a)<f(0)<f(b)
>
> 题源：880 第二章综合选择 21　|　题源类型：880　|　页码：p.198　|　模块：3.6 不等式
