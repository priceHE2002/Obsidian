...
> f(x) 设 在 连续，又 =1，则 f(x) x=0 lim |x| x→0 可导，f′(0)=0 可导，
> f′(0)̸=0 在 在 A. f(x) x=0 B. f(x) x=0 f′ (0),f′ f′ (0)̸
> =f′ f′ f′ 均存在但 与 不存在 C. (0) (0) D. (0) (0) − − − + + +
>
> 题源：26 版 660 数二第 533 题　|　题源类型：660　|　页码：p.130　|　模块：1.3 极限与导数
