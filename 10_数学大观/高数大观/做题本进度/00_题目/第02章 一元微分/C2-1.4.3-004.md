...
> |f(x )−f(x )|⩽K|x −x |2（∀x ∈(a,b)），则 设存在常数 使得 K >0 ,x 
> 2 1 2 1 1 2 在 有间断点 在 连续，但有不可导点 A. f(x) (a,b) B. f(x) (
> a,b) 可导，f′(x)̸≡0 可导，f′(x)≡0 在 在 C. f(x) (a,b) D. f(x) 
> (a,b)
>
> 题源：26 版 660 数二第 524 题（重点）　|　题源类型：660　|　页码：p.141　|　模块：1.4 某点的可导问题
