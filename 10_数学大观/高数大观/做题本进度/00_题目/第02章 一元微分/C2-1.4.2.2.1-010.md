...
备注:
---
# 1.4.2.2.1-010 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 x < x̸=0 , 1+e1/x 设 ，则下列正确的是 f(x)= : 0, x=0 不存在。 存在，
> 但 在 点不连续。 A. limf(x) B. limf(x) f(x) x=0 x→0 x→0 在 点连续
> ，但不可导。 在 点可导。 C. f(x) x=0 D. f(x) x=0
>
> 题源：26 版 660 数一二三第 151 题　|　题源类型：660　|　页码：p.136　|　模块：1.4 某点的可导问题
