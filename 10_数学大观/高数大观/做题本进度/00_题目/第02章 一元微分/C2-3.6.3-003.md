...
> 上二阶(cid:18)可导，且(cid:19)f′′(x)>0。证明： 【例 2.25】设 在 f(x) [
> a,b] x +x f(x )+f(x ) 1 2 1 2 ∈[a,b]，有 ； （I）对任意 < x <x
>  f 1 2 2 2 Z b a+b 1 f(a)+f(b) （II）（阿达马不等式）f < f(x)dx<
>  b−a 2 2 a
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.202　|　模块：3.6 不等式
