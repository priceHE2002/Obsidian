...
> p(x)=x3+ax2+bx+c，方程 设 有三个相异的实根 ，且 ，则 p(x)=0 x ,x ,x x 
> <x <x 1 2 3 1 2 3 p′(x )<0,p′(x )<0,p′(x p′(x )<0,p′(x
>  )>0,p′(x A. )>0 B. )<0 1 2 3 1 2 3 p′(x )>0,p′(x )<0,
> p′(x p′(x )>0,p′(x )>0,p′(x C. )>0 D. )<0 1 2 3 1 2 3 
> ·第 页，共 页· 183 402
>
> 题源：26 版 660 数二第 550 题　|　题源类型：660　|　页码：p.194　|　模块：3.5 3.5
