...
> 设函数f(x)在(−∞,+∞)上有定义，在区间[0,2]上，f(x)=x(x2−4)，若对任意的x都满足f(
> x)=kf(x+2)， 其中 为常数。 k [−2,0) 写出 在 上的表达式； (1) f(x) 问 为何
> 值时，f(x) 在 处可导？ (2) k x=0
>
> 题源：2004 数二　|　题源类型：真题　|　页码：p.141　|　模块：1.4 某点的可导问题
