...
> f′(x)>0，且 f′′(x)<0，则在 f(−x)=f(x)(−∞<x<+∞)，在 (−∞,0) (0,
> +∞) 若 内 内有（ ） (2) f′(x)>0，f′′(x)<0 f′(x)>0，f′′(x)>0 A.
>  B. f′(x)<0，f′′(x)<0 f′(x)<0，f′′(x)>0 C. D.
>
> 题源：1997 数三; 26 版 660 数二第 535 题　|　题源类型：真题　|　页码：p.121　|　模块：1.1 基本概念题
