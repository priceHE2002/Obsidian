...
> (−∞,+∞) 设 为 上的单调增加函数，则下列说法中，正确的是（ ） f(x) [f(x)]2 单调增加 
> A. [−f(−x)]3 单调增加 B. f′(x)>0 x∈(−∞,+∞)，都有 若 可导，则对任意 C.
>  f(x) x=f−1(y)，且 f−1(y) 若 可导，则 存在反函数 可导 D. f(x) f(x)
>
> 题源：900 第二章数二 A 类 9　|　题源类型：900　|　页码：p.172　|　模块：3.4 极最拐
