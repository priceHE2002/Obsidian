...
> f′(x)>0，f′′(x)>0，∆x 设函数 具有二阶导数，且 为自变量 在 处的增量，∆y 与 分 (7
> ) y =f(x) x x dy 0 别为 在点 处对应的增量与微分，若 ∆x>0，则 f(x) x ( )
>  0 A. 0<dy <∆y B. 0<∆y <dy C. ∆y <dy <0 D. dy <∆y <0
>
> 题源：2006 数一二三　|　题源类型：真题　|　页码：p.122　|　模块：1.2 微分概念
