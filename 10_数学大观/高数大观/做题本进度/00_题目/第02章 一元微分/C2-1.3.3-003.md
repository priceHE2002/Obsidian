...
> （6）设 在点 的某个邻域内有定义，则 在 处可导的一个充分条件是（ ） f(x) x=a f(x) x=a
>  f(a+2h)−f(a+h) 1 −f(a) 存在 存在 A. lim h f a+ B. lim h h
>  h→+∞ h→0 f(a+h)−f(a−h) f(a)−f(a−h) 存在 存在 C. lim D. li
> m 2h h h→0 h→0
>
> 题源：1989 数二　|　题源类型：真题　|　页码：p.128　|　模块：1.3 极限与导数
