...
> ( x=arctant, 【22】设可导函数 由 确定，则（ ）. y =y(x) =ln(1−t2)−si
> ny y 是 的极小值点 是 的极大值点 A. x=0 y =y(x) B. x=0 y =y(x) (−δ
> ,0)(δ 在 的邻域 内，y 单调 在 的邻域 内，y 单调 C. x=0 >0) =y(x) D. x=
> 0 (0,δ)(δ >0) =y(x) 递减 递增 ▶3.4.1.3.4 微分方程解的极值
>
> 题源：880 第二章基础选择 22　|　题源类型：880　|　页码：p.176　|　模块：3.4 极最拐
