...
> xf′′(x)+3x[f′(x)]2 =1−e−x. f′(x ̸=0), 已知函数 对一切 满足 若 则 
> y =f(x) x )=0 (x ( ). 0 0 是函数 的极大值 A. f(x ) f(x) 0 是函数
>  的极小值 B. f(x ) f(x) 0 是曲线 的拐点 C. (x ,f(x )) y =f(x) 0 
> 0 不是函数 的极值, 也不是曲线 的拐点 D. f(x ) f(x) (x ,f(x )) y =f(x)
>  0 0 0
>
> 题源：1997 数二; 26 版 1000 第五章强化 12　|　题源类型：真题　|　页码：p.177　|　模块：3.4 极最拐
