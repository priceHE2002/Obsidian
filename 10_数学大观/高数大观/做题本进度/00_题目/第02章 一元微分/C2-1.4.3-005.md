...
> 以下四个结论中正确的是： 是偶函数，f′ f′(0) [−a,a] 设 在 存在，则 存在。 A. f(x)
>  (0) + [−a,a] 设 在 是偶函数，则 是 的极值点。 B. f(x) x=0 f(x) 是奇函数
> ，f′ f′(0) [−a,a] 设 在 存在，则 存在。 C. f(x) (0) + ·第 页，共 页· 
> 130 402
>
> 题源：26 版 660 数二第 552 题（重点）　|　题源类型：660　|　页码：p.141　|　模块：1.4 某点的可导问题
