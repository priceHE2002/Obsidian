...
> f(x)=|x(1−x)|，则（ 设 ）． 是 的极值点，但点 不是曲线 的拐点 A. x=0 f(x) (
> 0,0) y =f(x) 不是 的极值点，但点 是曲线 的拐点 B. x=0 f(x) (0,0) y =f
> (x) 是 的极值点，且点 是曲线 的拐点 C. x=0 f(x) (0,0) y =f(x) 不是 的极值
> 点，点 也不是曲线 的拐点 D. x=0 f(x) (0,0) y =f(x) ·第 页，共 页· 174 
> 402
>
> 题源：2004 数二三　|　题源类型：真题　|　页码：p.185　|　模块：3.4 极最拐
