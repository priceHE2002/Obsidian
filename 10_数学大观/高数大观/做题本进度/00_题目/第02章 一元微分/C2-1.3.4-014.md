...
> (−1,1) 设函数 在区间 内有定义，且 limf(x)=0，则 (4) f(x) x→0 f(x) f(
> x) 时，f′(0)=m。 f′(0)=m 当 当 时，lim =m。 A. lim =m B. x x x
> →0 x→0 ′ ′ 时，f′(0)=m。 f′(0)=m 当 当 时，limf (x)=m。 C. lim
> f (x)=m D. x→0 x→0 ·第 页，共 页· 119 402
>
> 题源：2024 数一　|　题源类型：真题　|　页码：p.130　|　模块：1.3 极限与导数
