...
> 设 在 处连续，则 在 处可导的一个充分条件是（ ） f(x) x=a f(x) x=a f(a+x3)−f
> (a) 1 |x| −f(a) 存在 存在 A. lim f a+ B. lim |x| x2 x→∞ x→
> 0 f(a+∆x)−f(a−∆x) f(a+x3)−f(a) 存在 存在 C. lim D. lim tan
> x3 2∆x x→0 ∆x→0
>
> 题源：880 第二章基础选择 15　|　题源类型：880　|　页码：p.128　|　模块：1.3 极限与导数
