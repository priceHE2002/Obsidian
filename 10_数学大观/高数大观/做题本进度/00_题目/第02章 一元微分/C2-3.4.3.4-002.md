...
> Z Z x x et2 et2 dt·sin2x，则 已知函数 sintdt，g(x)= f(x)= 0 0
>  是 的极值点，也是 的极值点 A. x=0 f(x) g(x) 是 的极值点，(0,0) 是曲线 的拐点 
> B. x=0 f(x) y =g(x) 是 的极值点，(0,0) 是曲线 的拐点 C. x=0 f(x) y
>  =f(x) 是曲线 的拐点，也是曲线 的拐点 D. (0,0) y =f(x) y =g(x)
>
> 题源：2025 数一二三　|　题源类型：真题　|　页码：p.186　|　模块：3.4 极最拐
