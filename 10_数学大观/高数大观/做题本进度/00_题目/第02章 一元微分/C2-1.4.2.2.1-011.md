...
备注:
---
# 1.4.2.2.1-011 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 <1−cosx2 , x>0 设 ，其中 是有界函数，则 在 处 x3 f(x)= g(x) f(x) 
> x=0 : x⩽0 g(x)arcsin2x, 极限不存在. 极限存在，但不连续. A. B. 连续，但不可
> 导. 可导. C. D.
>
> 题源：26 版 660 数一二三第 163 题　|　题源类型：660　|　页码：p.136　|　模块：1.4 某点的可导问题
