...
> F′(x 可导，F(x)=g(x)|f(x)|，又 设 在 )=0，则 存在的充要条件是： f(x),g(x
> ) x=x f(x ) 0 0 0 f′(x A. )=0 B. g(x )=0 0 0 )f′(x 以上均
> 不对 C. g(x )=0 D. 0 0
>
> 题源：26 版 660 数二第 526 题（重点）　|　题源类型：660　|　页码：p.133　|　模块：1.4 某点的可导问题
