...
> ( x=2t+|t| 设函数 由 确定，则（ ）. y =f(x) =|t|sint y 连续，f′(0) 
> f′(0) 存在，f′(x) 不存在 在 处不连续 A. f(x) B. x=0 f′(x) 连续，f′′(
> 0) f′′(0) 存在，f′′(x) 不存在 在 处不连续 C. D. x=0 ·第 页，共 页· 129
>  402
>
> 题源：2023 数一二　|　题源类型：真题　|　页码：p.140　|　模块：1.4 某点的可导问题
