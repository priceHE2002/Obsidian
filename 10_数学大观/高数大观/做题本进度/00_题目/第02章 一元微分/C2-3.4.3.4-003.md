...
> ( 2−cosx, x⩽0 设 ，则 √ f(x)= x+1, x>0 是 的极值点，但 不是曲线 的拐点。
>  A. x=0 f(x) (0,1) y =f(x) 不是 的极值点，但 是曲线 的拐点。 B. x=0 f
> (x) (0,1) y =f(x) 是 的极值点，且 是曲线 的拐点。 C. x=0 f(x) (0,1) 
> y =f(x) 不是 的极值点，(0,1) 不是曲线 的拐点。 D. x=0 f(x) y =f(x)
>
> 题源：26 版 660 数一二三第 172 题　|　题源类型：660　|　页码：p.186　|　模块：3.4 极最拐
