...
> f′′(x)>0，则 f′(1)，f′(0)，f(1)−f(0) f(0)−f(1) 设函数 在 上 或 的
> 大小顺序是（） (4) f(x) [0,1] f′(1)>f′(0)>f(1)−f(0) f′(1)>f(1
> )−f(0)>f′(0) A. B. f(1)−f(0)>f′(1)>f′(0) f′(1)>f(0)−f(
> 1)>f′(0) C. D.
>
> 题源：1995 数一二　|　题源类型：真题　|　页码：p.195　|　模块：3.6 不等式
