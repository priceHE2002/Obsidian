...
> 设 在 的某邻域内有定义，在 的某去心邻域内可导，下述论断正确的是（）. f(x) x=a x=a ′ ′ 
> f′(a)=A f′(a)=A，则 若 (x)=A，则 若 A. lim f B. lim f (x)=A 
> x→a x→a ′ ′ f′(a) f′(a) (x)=∞，则 (x)=∞ 若 不存在 若 不存在，则 C.
>  lim f D. lim f x→a x→a
>
> 题源：26 版 1000 第三章强化 17　|　题源类型：1000　|　页码：p.129　|　模块：1.3 极限与导数
