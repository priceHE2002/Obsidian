...
> f′(x) 上单调增加，f(0)=f′(0)=0，则在 设 在 上（ ）。 [0,1] [0,1] 单调增加
> ，e−xf(x) 单调增加，e−xf(x) exf(x) exf(x) 单调减少 单调增加 A. B. 单调
> 减少，e−xf(x) 单调减少，e−xf(x) exf(x) exf(x) 单调增加 单调减少 C. D. 
> ▶3.4.1.2 极值概念题
>
> 题源：26 版 1000 第五章强化 11　|　题源类型：1000　|　页码：p.172　|　模块：3.4 极最拐
