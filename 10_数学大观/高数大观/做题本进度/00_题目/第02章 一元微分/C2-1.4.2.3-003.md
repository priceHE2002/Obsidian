...
> ( x=t|t|, 设函数 由 确定，则（） y =f(x) =|t|sint2 y 连续，f′(0) f′
> (0) 存在，f′(x) 不存在。 在 处不连续。 A. f(x) B. x=0 f′(x) 连续，f′′(
> 0) f′′(0) 存在，f′′(x) 不存在。 在 处不连续。 C. D. x=0
>
> 题源：900 第二章数二 A 类 29　|　题源类型：900　|　页码：p.141　|　模块：1.4 某点的可导问题
