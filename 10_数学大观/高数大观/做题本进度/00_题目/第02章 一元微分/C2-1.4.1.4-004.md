...
> F(x)=f(x)|sinx| 【5】设 在 的某邻域内有定义，则 在 处可导的充分必要条件是（ ）。 f(
> x) x=0 x=0 存在 A. limf(x) x→0 B. limf(x)=f(0) x→0 在 处可导
>  C. f(x) x=0 f(x)=− 与 均存在，且 D. lim f(x) lim f(x) lim l
> im f(x) x→0− x→0− x→0+ x→0+ ·第 页，共 页· 121 402
>
> 题源：880 第二章综合选择 5　|　题源类型：880　|　页码：p.132　|　模块：1.4 某点的可导问题
