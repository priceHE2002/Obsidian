...
> f(x) 设函数 在区间 上具有二阶导数，且 f(1)>0， <0。证明： f(x) [0,1] lim x
>  x→0+ （I）方程 在区间 内至少存在一个实根； f(x)=0 (0,1) f(x)f′′(x)+[f′
> (x)]2 （II）方程 在区间 内至少存在两个不同实根。 =0 (0,1)
>
> 题源：2017 数一二　|　题源类型：真题　|　页码：p.190　|　模块：3.5 3.5
