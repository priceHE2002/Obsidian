...
> (−∞,+∞) ̸=0 设函数 在 内有定义，x 是函数 的极大值点，则（ ） f(x) f(x) 0 −x
>  −f(−x) 必是 的驻点 必是 的极小值点 A. x f(x) B. 0 0 −x −f(x) f(x)
> ≤f(x 必是 的极小值点 对一切 都有 C. D. x ) 0 0
>
> 题源：1991 数二　|　题源类型：真题　|　页码：p.173　|　模块：3.4 极最拐
