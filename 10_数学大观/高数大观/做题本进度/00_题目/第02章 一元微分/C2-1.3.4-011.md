...
> 设函数 在 处连续，以下命题错误的选项是 (2) f(x) x=0 ( ) f(x)+f(−x) f(x) 
> 假设 存在，则 假设 存在，则 A. lim f(0)=0 B. lim f(0)=0 x x x→0 x→
> 0 f(x)−f(−x) f(x) f′(0) f′(0) 存在，则 存在 假设 存在，则 存在 假设 D.
>  lim C. lim x x x→0 x→0
>
> 题源：2007 数一二三　|　题源类型：真题　|　页码：p.130　|　模块：1.3 极限与导数
