...
备注:
---
# 1.4.2.2.1-008 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 < x⩽0, x, 已知函数 则 (4) f(x)= ( ) 1 1 :1 <x⩽ ,n=1,2,···
>  , , n n+1 n 是 的第一类间断点 是 的第二类间断点 A. x=0 f(x) B. x=0 f(
> x) 在 处连续但不可导 在 处可导 C. f(x) x=0 D. f(x) x=0
>
> 题源：2016 数一　|　题源类型：真题　|　页码：p.135　|　模块：1.4 某点的可导问题
