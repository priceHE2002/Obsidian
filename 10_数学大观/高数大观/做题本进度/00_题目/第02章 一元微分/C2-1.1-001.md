...
> f′(x)>0，f′′(x)>0，则 f(x)=−f(−x)，在 (0,+∞) (−∞,0) （5）若 内（
>  ） 内 在 f(x) f′(x)<0，f′′(x)<0 f′(x)<0，f′′(x)>0 A. B. f′
> (x)>0，f′′(x)<0 f′(x)>0，f′′(x)>0 C. D.
>
> 题源：1993 数二; 880 第二章基础选择 9　|　题源类型：真题　|　页码：p.121　|　模块：1.1 基本概念题
