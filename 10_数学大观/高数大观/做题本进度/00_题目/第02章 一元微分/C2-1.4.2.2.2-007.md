...
> [!note]- 题干预览（自动提取，仅供参考）
> 8 < |x|asin 设 f(x)= : 0, 为连续函数； (I)f(x) 为可导函数； ()f(x) 为连续可导函数， (III)f(x) 则参数 必须分别满足 a (I)a>0；(II)a>1；(III)a>2 A. (I)a>0；(II)a⩾1；(III)a⩾2 C.
>
> 题源：26 版 660 数二第 544 题; 880 第二章综合解答 5　|　页码：p.139　|　模块：1.4 某点的可导问题
