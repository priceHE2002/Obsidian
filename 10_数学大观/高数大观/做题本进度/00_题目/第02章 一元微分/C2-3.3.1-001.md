...
> f′′(x （5）设函数 具有二阶连续导数，且 )<0(i=1,2)。若两条曲线 在点 f (x)(i=1,
> 2) y =f (x)(i=1,2) i 0 i i 处具有公切线 g(x)，且在该点处曲线 的曲率大于曲线
>  的曲率，则在 的 (x ,y ) y = y = f (x) y = f (x) x 0 0 1 2 0 
> 某个邻域内，有（ ） (x)≤f (x)≤g(x) (x)≤f (x)≤g(x) A. f B. f 1 2
>  2 1 (x)≤g(x)≤f (x)≤g(x)≤f C. f (x) D. f (x) 1 2 2 1
>
> 题源：2016 数二　|　题源类型：真题　|　页码：p.167　|　模块：3.3 曲率
