...
> f′(x)>f(x)，且 [0,+∞) 时，f(x)>ex。 （I）设 在 上可导，满足 f(0)=1。证明
> ：当 f(x) x>0 （II）设f(x)在[0,+∞)上二阶可导，满足f′′(x)>f(x)，且f(0)=
> f′(0)=1。证明：当x>0时，f(x)>ex。 ·第 页，共 页· 185 402
>
> 题源：东南大学 2014（重点）　|　题源类型：其他　|　页码：p.196　|　模块：3.6 不等式
