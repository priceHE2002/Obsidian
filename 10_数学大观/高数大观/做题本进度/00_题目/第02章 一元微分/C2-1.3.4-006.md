...
> [0,+∞) 设函数 在 上二阶可导，且 f(0)=0。下列命题中，正确命题的个数为（） f(x) f(x)
>  f′′(x)<0，则 ⃝ (0,+∞) (0,+∞) 若在 上恒有 在 上单调减少。 1 x f(x) f
> ′′(x)⩽0。 ⃝ 若 在 上单调减少，则在 上恒有 (0,1] (0,1] 2 x f(x) ′ ⃝ 若
>  存在，则 存在。 lim lim f (x) 3 x x→+∞ x→+∞ f(x) ′ f′′(x)<0，
> 且 ⃝ (0,+∞) 若在 上恒有 存在，则 存在。 lim lim f (x) 4 x x→+∞ x→+∞
>  A. 1 B. 2 C. 3 D. 4
>
> 题源：900 第二章数二 B 类 18（重点）　|　题源类型：900　|　页码：p.129　|　模块：1.3 极限与导数
