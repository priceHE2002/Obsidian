...
> a>e。证明：(a+x)a <aa+x。 设 x>0，常数 1 < 2 27-3 lnb−lna 1 √ <
>  < b−a ab 4 (b−a)。 e2 x2 (−1<x<1)。 2 1 1 p 1+ < x x(1+
> x) x 1 ⩽4，当且仅当 时等号成立。 x=1 1+ x ·第 页，共 页· 189 402
>
> 题源：1993 数二　|　题源类型：真题　|　页码：p.200　|　模块：3.6 不等式
