...
> (−1,1) 设函数 在区间 内有定义，且在点 处连续，则以下结论： f(x) x=0 f(x) √ ⃝ 当
>  时，f(x) 在点 处可导； lim =0 x=0 1 3x x→0 f(x) ⃝ 当 时，f(x) 在点
>  处可导； lim =0 x=0 2 x2 x→0 f(x) √ ⃝ 当 在点 处可导时，lim =0。 f
> (x) x=0 3 3x x→0 所有正确结论的序号为（ ）。 ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ A. B. C. D
> . 1 2 2 3 1 2
>
> 题源：26 版 1000 第三章强化 4　|　题源类型：1000　|　页码：p.130　|　模块：1.3 极限与导数
