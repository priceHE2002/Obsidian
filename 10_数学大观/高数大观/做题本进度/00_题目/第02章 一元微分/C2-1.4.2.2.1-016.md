...
备注:
---
# 1.4.2.2.1-016 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> (√ x⩾0 x, 设 ，则 √ f(x)= −x, x<0 f′(0) 在 不连续. 存在. A. f(x
> ) x=0 B. f′(0) f′(0) 不存在，曲线 在 不存在切线. 不存在，曲线 在 有切线. C. 
> y =f(x) (0,0) D. y =f(x) (0,0)
>
> 题源：26 版 660 数二第 529 题　|　题源类型：660　|　页码：p.137　|　模块：1.4 某点的可导问题
