...
源题号: "3.6.1-005"
序: 1501
题源: "2000 数一二; 26 版 1000 第六章基础 17"
题源大类: "真题"
重点: false
页码: 195
状态: "未开始"
...
> f′(x)g(x)−f(x)g′(x)<0，则当 设函数 是大于零的可导函数，且 时，有（ ）． (3) f
> (x),g(x) a<x<b A. f(x)g(b)>f(b)g(x) B. f(x)g(a)>f(a)g(
> x) C. f(x)g(x)>f(b)g(b) D. f(x)g(x)>f(a)g(a)
>
> 题源：2000 数一二; 26 版 1000 第六章基础 17　|　题源类型：真题　|　页码：p.195　|　模块：3.6 不等式
