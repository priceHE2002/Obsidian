...
备注:
---
# 1.4.2.2.1-014 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> (cid:0)√ (cid:1)(cid:3) 8 >< t2+2e+t sin tx x̸=0, lim 
> , xarctant t→−∞ 若函数 则下列说法中，正确的是（ ） f(x)= >:2e , x=0, π
>  在 处的极限存在，但不连续 在 处连续，但不可导 A. f(x) x=0 B. f(x) x=0 f′(0
> )=0 f′(0)̸=0 在 处可导，且 在 处可导，且 C. f(x) x=0 D. f(x) x=0
>
> 题源：900 第二章数二 A 类 7　|　题源类型：900　|　页码：p.136　|　模块：1.4 某点的可导问题
