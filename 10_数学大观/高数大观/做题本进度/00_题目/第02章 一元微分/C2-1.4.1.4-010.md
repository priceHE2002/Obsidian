...
> f(x)−f(x (x−x 设函数 在 某邻域有定义，则存在函数 在 处连续并使 是 f(x) x = x 
> g(x) x ) = )g(x) 0 0 0 0 在 处可导的 f(x) x=x 0 充分而非必要条件 必要
> 而非充分条件 A. B. 充分且必要条件 既非充分也非必要条件 C. D. 研究具体函数在某点的导数
>
> 题源：26 版 660 数二第 525 题　|　题源类型：660　|　页码：p.133　|　模块：1.4 某点的可导问题
