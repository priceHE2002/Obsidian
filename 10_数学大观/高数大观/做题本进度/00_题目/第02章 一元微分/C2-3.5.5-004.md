...
> f′′−kf′−f 设 为任意非零常数，函数 在 上满足 =0，且 f(a)=f(b)=0，则下列说法中，正
>  k f(x) [a,b] 确的是（ ） 在 上先单调减少，再单调增加 在 上先单调增加，再单调减少 A. 
> f(x) [a,b] B. f(x) [a,b] 在 上恒为零 在 内恰有一个零点 C. f(x) [a,b
> ] D. f(x) (a,b) 不等式
>
> 题源：900 第二章数二 B 类 10　|　题源类型：900　|　页码：p.195　|　模块：3.5 3.5
