...
> 8 ><x=t|t|, Z |t| 确定，则下列选项中正确的是（ ）. 【8】设 由 y =f(x) >:y
>  eu2 =|t| du 0 f′(x) 在 处连续 在 处不连续 A. x=0 B. f(x) x=0 f
> ′(0) f′(0) 不存在 存在 C. D. 研究抽象函数在某点的导数
>
> 题源：880 第二章综合选择 8　|　题源类型：880　|　页码：p.141　|　模块：1.4 某点的可导问题
