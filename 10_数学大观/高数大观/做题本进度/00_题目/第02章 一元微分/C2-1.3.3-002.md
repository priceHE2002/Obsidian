...
> 设函数 满足 f(0)=0，则 在 处可导的充分必要条件为（ ） f(x) f(x) x=0 1 1 f(t
> anh−h) f(ln(1+h)−h) 存在 存在 B. lim A. lim h→0h3 h→0h2 1 
> 1 f(arctanh−h) [f(h)−f(−h)] 存在 存在 D. C. lim lim h→0h h
> →0h ·第 页，共 页· 116 402
>
> 题源：900 第二章数二 B 类 2　|　题源类型：900　|　页码：p.127　|　模块：1.3 极限与导数
