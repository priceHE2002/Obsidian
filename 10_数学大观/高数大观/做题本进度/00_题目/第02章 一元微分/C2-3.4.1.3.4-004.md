...
> √ 3xf′′(x)+xf′(x)=e−x−1，若 f′(x ̸=0)，则（ 已知函数 =f(x)，对一切 
> 满足 ）。 y x )=0(x 0 0 是 的极小值点 是 的极大值点 A. x=x f(x) B. x=x
>  f(x) 0 0 是曲线 的拐点 以上结论均不正确 C. (x ,f(x )) y =f(x) D. 0 
> 0
>
> 题源：26 版 1000 第五章基础 17　|　题源类型：1000　|　页码：p.177　|　模块：3.4 极最拐
