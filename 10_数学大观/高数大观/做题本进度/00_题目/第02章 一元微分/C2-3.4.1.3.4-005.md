...
> 设f(x)对一切x∈(−∞,+∞)满足方程(x−1)f′′(x)+2(x−1)[f′(x)]3 =1−e1−
> x，且f(x)在x=a(a̸=1) f′(a)=0，则 处 x=a 是 的极小值点. 是 的极大值点. A.
>  f(x) B. f(x) 不是 的极值点. 是 的拐点. C. f(x) D. f(x)
>
> 题源：25 版 660 数一二三第 159 题　|　题源类型：660　|　页码：p.177　|　模块：3.4 极最拐
