...
备注:
---
# 1.4.2.2.1-006 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 <1− cosx √ , x>0, x （2）设 其中 是有界函数，则 在 处（ ） f(x)= g(x
> ) f(x) x=0 : x⩽0, x2g(x), 极限不存在 极限存在，但不连续 A. B. 连续，但不可
> 导 可导 C. D.
>
> 题源：1999 数一二; 880 第二章基础选择 1　|　题源类型：真题　|　页码：p.135　|　模块：1.4 某点的可导问题
