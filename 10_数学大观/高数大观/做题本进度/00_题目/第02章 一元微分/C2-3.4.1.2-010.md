...
> )=0，f′(x )g′(x 设 在 处可导，且 )<0，则（ ）。 f(x),g(x) x=x f(x )
> =g(x 0 0 0 0 0 不是 的驻点 是 的驻点，但不是极值点 A. x f(x)g(x) B. x 
> f(x)g(x) 0 0 是 的驻点也是极小值点 是 的驻点也是极大值点 C. x f(x)g(x) D. 
> x f(x)g(x) 0 0
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.174　|　模块：3.4 极最拐
