...
> (−δ,δ) x∈(−δ,δ) |f(x)|⩽x2，则 设函数 在区间 内有定义，若当 时，恒有 必是 的（
>  (2) f(x) x=0 f(x) ） 间断点. 连续而不可导的点. A. B. f′(0)=0. f′(
> 0)̸=0. 可导的点，且 可导的点，且 C. D.
>
> 题源：1996 数二; 880 第二章基础选择 18　|　题源类型：真题　|　页码：p.141　|　模块：1.4 某点的可导问题
