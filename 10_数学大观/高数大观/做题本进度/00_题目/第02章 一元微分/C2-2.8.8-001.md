...
> f′(x) [f(x)]2，则当 （3）已知函数 具有任意阶导数，且 为大于 的正整数时，f(x) 的 阶导
> 数 f(x) = n 2 n f(n)(x) 为（ ） n![f(x)]n+1 n[f(x)]n+1 [f(
> x)]2n n![f(x)]2n A. B. C. D.
>
> 题源：1990 数一二; 880 第二章基础选择 16　|　题源类型：真题　|　页码：p.156　|　模块：2.8 高阶导
