...
> 设 f(x)=g(x)φ(x)，其中 在 邻域 有定义，g(x) 在 连续，φ(x) 在 不连 g(x),φ
> (x) x=x U x=x x=x 0 0 0 续，但在 有界，则 是 在 连续的 U g(x )=0 f(
> x) x=x 0 0 充要条件 充分非必要条件 A. B. 必要非充分条件 既非充分也非必要条件 C. D.
>
> 题源：26 版 660 数二第 523 题（重点）　|　题源类型：660　|　页码：p.133　|　模块：1.4 某点的可导问题
