...
备注:
---
# 1.4.2.2.2-006 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 1 < x̸=0, xksin , x 【7】设 f(x)= : 0, x=0. （I）当 为何值时，f
> (x) 在 处不可导； k x=0 （II）当 为何值时，f(x) 在 处可导，但导函数不连续； k x=0
>  （III）当 为何值时，f(x) 在 处导函数连续。 k x=0 ·第 页，共 页· 127 402
>
> 题源：880 第二章基础解答 7　|　题源类型：880　|　页码：p.138　|　模块：1.4 某点的可导问题
