...
> (−1,1) 设函数 在区间 内有定义，且 limf(x)=0，则（） f(x) x→0 f(x) f(x)
>  p 当 时，f(x) 在 处可导。 当 时，f(x) 在 处可导。 A. lim =0 x=0 B. li
> m =0 x=0 |x| x2 x→0 x→0 f(x) f(x) p 当 在 处可导时，lim =0。 当
>  在 处可导时，lim =0。 C. f(x) x=0 D. f(x) x=0 |x| x2 x→0 x→0
>
> 题源：2020 数一　|　题源类型：真题　|　页码：p.130　|　模块：1.3 极限与导数
