...
> g(x) ′ −3，f ln(1+x2)− 设 在 的某邻域内连续，f(x) 具有一阶连续导数，且满足 g(
> x) x = 0 lim = (x) = x x→0 Z 1 g(xt)dt，则（ ）。 x 0 是 的极大
> 值点 是 的极小值点 A. x=0 f(x) B. x=0 f(x) 是曲线 的拐点 以上结论均不正确 C.
>  (0,f(0)) y =f(x) D.
>
> 题源：26 版 1000 第五章基础 16（重点）　|　题源类型：1000　|　页码：p.187　|　模块：3.4 极最拐
