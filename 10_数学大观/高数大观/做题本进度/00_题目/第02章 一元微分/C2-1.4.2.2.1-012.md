...
备注:
---
# 1.4.2.2.1-012 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 >< 1 x̸=0, x2 2+sin , x 设函数 且函数 在 处可导，则函数 在 处（） φ(x)
> = f(x) x=0 f(φ(x)) x=0 >: 0, x=0, 不连续 连续但不可导 A. B. 可导且
> 导数为 可导且导数不为 C. 0 D. 0
>
> 题源：900 第二章数二 A 类 2　|　题源类型：900　|　页码：p.136　|　模块：1.4 某点的可导问题
