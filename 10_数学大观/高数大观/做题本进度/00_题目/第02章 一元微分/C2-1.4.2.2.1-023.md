...
备注:
---
# 1.4.2.2.1-023 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> (−∞,0)∪(0,+∞) −1， 设 为定义在 上的分段连续函数，且 1，则 f(x) lim f(x) 
> = lim f(x) = F(x) = Z x→0− x→0+ x (sinx−sint)f(t)dt 在 
> 处可导的最高阶数为 x=0 . 0 ▶1.4.2.2.2 导函数连续
>
> 题源：900 第二章数二 B 类 22　|　题源类型：900　|　页码：p.138　|　模块：1.4 某点的可导问题
