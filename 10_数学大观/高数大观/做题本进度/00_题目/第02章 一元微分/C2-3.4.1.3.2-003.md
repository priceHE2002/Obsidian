...
> 8 >>>< −x3lnx, x>0, 0, x=0, 设函数 则（ ） f(x)= >>>:arctan 
> 1 π x+ + , x<0, x 2 有两个极大值点，无极小值点 有一个极大值点，一个极小值点 A. f(
> x) B. f(x) 有两个极大值点，一个极小值点 有一个极大值点，两个极小值点 C. f(x) D. f(
> x)
>
> 题源：900 第二章数二 A 类 10　|　题源类型：900　|　页码：p.176　|　模块：3.4 极最拐
