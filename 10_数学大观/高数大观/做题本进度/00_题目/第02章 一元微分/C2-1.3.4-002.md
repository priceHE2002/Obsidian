...
> (0,+∞) 设函数 在 内有界且可导，则（ ） y =f(x) ′ ′ ′ 当 时，必有 (x)=0。 当
>  存在时，必有 (x)=0。 A. lim f(x)=0 lim f B. lim f (x) lim f 
> x→+∞ x→+∞ x→+∞ x→+∞ ′ ′ ′ 当 时，必有 (x)=0。 当 存在时，必有 (x)=0
> 。 C. lim f(x)=0 lim f D. lim f (x) lim f x→0+ x→0+ x→0
> + x→0+ ·第 页，共 页· 117 402
>
> 题源：2002 数一二　|　题源类型：真题　|　页码：p.128　|　模块：1.3 极限与导数
