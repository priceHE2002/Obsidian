...
> [u(x)v(x)]′ =u′(x)v(x)+u(x)v′(x)； （I）设函数 可导，利用导数定义证明 u
> (x),v(x) (x),··· (x)···u （II）设函数 可导，f(x)=u (x)，写出 的求导公
> 式。 u (x),u ,u (x) (x)u f(x) 1 2 n 1 2 n 微分概念
>
> 题源：2015 数一三　|　题源类型：真题　|　页码：p.122　|　模块：1.1 基本概念题
