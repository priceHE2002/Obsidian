...
> f′(0)=b，其中 （2）设函数 对任意 均满足等式 f(1+x)=af(x)，且有 为非零常数，则（ ）
>  f(x) x a,b f′(1)=a 在 处不可导 在 处可导，且 A. f(x) x=1 B. f(x)
>  x=1 f′(1)=b f′(1)=ab 在 处可导，且 在 处可导，且 C. f(x) x=1 D. f
> (x) x=1
>
> 题源：1990 数三; 26 版 660 数二第 522 题　|　题源类型：真题　|　页码：p.141　|　模块：1.4 某点的可导问题
