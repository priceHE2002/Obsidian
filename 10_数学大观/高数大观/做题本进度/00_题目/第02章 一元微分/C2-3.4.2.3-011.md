...
> Z x f(t)dt， 【11】设 在 上可导，f(x) 在 处取得最小值，在 处取得最大值．F(x) f(
> x) [a,b] x = a x = b = 0 x∈[a,b]，则（ ）． F′′(b)⩾0 F′′(a)
> ⩾0 F′′(b)<0 F′′(a)>0 且 且 B. A. − − + + F′′(b)>0 F′′(a)
> <0 F′′(b)<0 F′′(a)<0 且 且 D. C. − − + + 拐点
>
> 题源：880 第二章综合选择 11　|　题源类型：880　|　页码：p.182　|　模块：3.4 极最拐
