...
> 1 设函数 ，则下列说法中，正确的是（） f(x)= 2x3−5x2+4x f(x)恰有一个单调增加区间，两
> 个单调减少区间。 恰有一个单调增加区间，三个单调减少区间。 A. B. f(x) 恰有一个单调减少区间，两个
> 单调增加区间。 f(x)恰有一个单调减少区间，三个单调增加区间。 C. f(x) D.
>
> 题源：900 第二章数二 A 类 8　|　题源类型：900　|　页码：p.172　|　模块：3.4 极最拐
