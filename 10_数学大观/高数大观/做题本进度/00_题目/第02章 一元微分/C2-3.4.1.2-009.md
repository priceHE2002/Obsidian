...
> f′(x )=f′′(x )=f′′′(x f(4)(x 【10】设 在 的某邻域内有四阶连续导数，且 )=
> 0，且 )<0，则 y =f(x) x 0 0 0 0 0 （ ）。 在 处取得极小值 在 处取得极大值 A
> . f(x) x B. f(x) x 0 0 是 的拐点 在 的某邻域内单调减少 C. (x ,f(x ))
>  y =f(x) D. f(x) x 0 0 0
>
> 题源：880 第二章综合选择 10　|　题源类型：880　|　页码：p.174　|　模块：3.4 极最拐
