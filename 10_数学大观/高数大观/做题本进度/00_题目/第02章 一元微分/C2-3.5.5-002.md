...
> 具有二阶导数，f(0)>0，f′(0)<0，当 时，f′′(x)<0，则下列说法中，正确的是（ x⩾0 设函
> 数 ） f(x) f(0) f(−f(0)f′(0))>0 − A. B. f >0 f′(0) [0,+∞
> ) [0,+∞) 在 上只有一个零点 在 上没有零点 C. f(x) D. f(x)
>
> 题源：900 第二章数二 B 类 16　|　题源类型：900　|　页码：p.194　|　模块：3.5 3.5
