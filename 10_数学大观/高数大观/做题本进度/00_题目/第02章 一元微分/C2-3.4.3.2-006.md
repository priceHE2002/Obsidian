...
> (−∞,+∞) ̸=0，(x 设 在 内可导，x 是 的拐点，则 f(x) ,f(x )) y =f(x) 
> 0 0 0 f′(x) (−x ,−f(x =−f(−x) 必是 的驻点 必是 的拐点 A. x B. ))
>  y 0 0 0 (−x ,−f(−x =−f(x) 必是 的拐点 对任意 与 ，y 的凹凸性相反 C. )
> ) y D. x>x x<x =f(x) 0 0 0 0
>
> 题源：26 版 660 数二第 556 题　|　题源类型：660　|　页码：p.184　|　模块：3.4 极最拐
