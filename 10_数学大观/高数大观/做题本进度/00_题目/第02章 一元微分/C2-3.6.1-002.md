...
> f′′(x)>0，f(0)=f(1)。当 x∈(0,1) 【20】设 在 上二阶可导，且 时，下列结论正确的
> 是（ ） f(x) [0,1] （1）(1−x)[f(x)−f(0)]<x[f(1)−f(x)]； （2）(
> 1−x)[f(x)−f(0)]>x[f(1)−f(x)]； （3）x[f(x)−f(0)]<(1−x)[f(
> 1)−f(x)]； （4）x[f(x)−f(0)]>(1−x)[f(1)−f(x)]。 （1）（4） （2）
> （3） （2）（4） （1）（3） A. B. C. D.
>
> 题源：880 第二章综合选择 20　|　题源类型：880　|　页码：p.195　|　模块：3.6 不等式
