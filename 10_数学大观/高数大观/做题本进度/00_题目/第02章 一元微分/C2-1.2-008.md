...
> [!note]- 题干预览（自动提取，仅供参考）
> 设函数 f(x) 微分，则下列说法中，正确的是（ 若在 的某邻域内有 A. x 0 若在 的某邻域内有 B. x 0 f′′(x) 若 在 C. f′′(x) 若 在 D. 极限与导数 1.3 已知极限求导数 1.3.1 ▶1.3.1.1 求一阶导
>
> 题源：900 第二章数二 B 类 4　|　页码：p.123　|　模块：1.2 微分概念
