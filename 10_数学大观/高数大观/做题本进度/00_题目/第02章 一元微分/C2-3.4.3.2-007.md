...
> 【例 2.14】下列命题中正确的是（ ）. 设 在 内只有一个驻点 ，且 是 的极小值点，则 是 的最小值 
> A. f(x) (a,b) x x=x f(x) f(x ) f(x) 0 0 0 设 是 的极值点，则点 
> 不是曲线 的拐点 B. x=x f(x) (x ,f(x )) y =f(x) 0 0 0 f′(x )=0
> ，f′′(x 设 在 处二阶可导，x=x 是 的极小值点，则 C. f(x) x=x f(x) )>0 0 
> 0 0 0 f′ 设 (b)<0，则 不是 在 上的最大值 D. f(b) f(x) [a,b] − ▶3.
> 4.3.3 求拐点（个数）
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.184　|　模块：3.4 极最拐
