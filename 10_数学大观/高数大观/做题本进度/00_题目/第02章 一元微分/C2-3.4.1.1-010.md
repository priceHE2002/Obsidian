...
> （3）设 在 处有二阶导数，则（ ） f(x) x=x 0 的某邻域内单调增加时，f′(x f′(x 在 当
>  在 的某邻域内单调增加 当 时，f(x) A. f(x) )>0 )>0 x B. x 0 0 0 0 的
> 某邻域内是凹函数时，f′′(x f′′(x 当 在 当 时，f(x) 在 的某邻域内是凹函 C. f(x) 
> x )>0 D. )>0 x 0 0 0 0 数
>
> 题源：2022 数二　|　题源类型：真题　|　页码：p.171　|　模块：3.4 极最拐
