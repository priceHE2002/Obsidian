...
> 1 ，其中 可导，求 dy； y =φ arctan φ x d2z dz y+ey 满足 =x，f,φ 均
> 具有二阶导数，求 , dx2 dx . ef1(x)f2(x)。令 的某邻域内均可导，其中 x = 0 f 
> (x) > 0,f (x) > 0,g(x) = 1 2 ） 2 φ′(0)>1 φ′(0)=1 C. D.
>
> 题源：880 第二章基础解答 3-1　|　题源类型：880　|　页码：p.145　|　模块：2.2 复合函数求导
