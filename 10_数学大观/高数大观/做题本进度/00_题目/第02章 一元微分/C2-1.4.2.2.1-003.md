...
备注:
---
# 1.4.2.2.1-003 · 某点的可导问题

> [!question] 题目（程序自动提取，公式可能有偏差）
> 8 <2 x⩽1, x3, 3 （2）设 则 在 处的（ ） f(x)= f(x) x=1 : x2, x>
> 1, 左、右导数都存在. 左导数存在，但右导数不存在. A. B. 左导数不存在，但右导数存在. 左、右导数
> 都不存在. C. D. ·第 页，共 页· 123 402
>
> 题源：1994 数二　|　题源类型：真题　|　页码：p.134　|　模块：1.4 某点的可导问题
