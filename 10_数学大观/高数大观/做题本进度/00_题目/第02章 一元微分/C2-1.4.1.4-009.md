...
> 的跳跃间断点，g′(a) g(a)=0，g′(a)=0 设连续函数 F(x)=g(x)φ(x)，x=a 是 
> 存在，则 是 在 φ(x) F(x) 处可导的 x=a 充分必要条件 充分非必要条件 A. B. 必要非充分
> 条件 非充分非必要条件 C. D.
>
> 题源：26 版 660 数二第 527 题（重点）　|　题源类型：660　|　页码：p.133　|　模块：1.4 某点的可导问题
