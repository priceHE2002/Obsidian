...
> [0,+∞) 设函数 在区间 上可导，则（ ） f(x) ′ 当 存在时， 存在。 A. lim f(x) 
> lim f (x) x→+∞ x→+∞ ′ 当 存在时， 存在。 B. lim f (x) lim f(x)
>  x→+∞R x→+∞ x f(t)dt 0 当 存在时， 存在。 C. lim lim f(x) x x→
> +∞ x→R+∞ x f(t)dt 0 当 存在时， 存在。 D. lim f(x) lim x x→+∞ 
> x→+∞
>
> 题源：2025 数一　|　题源类型：真题　|　页码：p.129　|　模块：1.3 极限与导数
