...
> f′(x)>0，f(0)=1，则当 x∈(0,1) 设函数 在区间 上连续，在区间 内可导，且 时，有（）。
>  f(x) [0,1] (0,1) f(x)<ef(x)−1 f(x)>ef(x)−1 A. B. f(x)
> <e−f(x)+1 f(x)>ef(x)+1 C. D.
>
> 题源：26 版 1000 第六章强化 20　|　题源类型：1000　|　页码：p.198　|　模块：3.6 不等式
