...
> |x||x|−x 下列关于函数 的说法中，正确的是（） f(x)= |x||x|ln|x| 只有可去间断点 
> 只有跳跃间断点 A. f(x) B. f(x) 有可去间断点和无穷间断点，没有跳跃间断 有可去间断点、跳跃间
> 断点和无穷间断点 C. f(x) D. f(x) 点
>
> 题源：900 数二第一章 A 类 21　|　题源类型：900　|　页码：p.117　|　模块：3.3 间断点
