...
> {x } {y } 设数列 与 满足 =1，则下列结论中，正确的是（ ） lim x y n n n n n
> →∞ {x } {y } {x } {y } 若 发散，则 必收敛 若 无界，则 必有界 A. B. n n
>  (cid:26)n n 1 {x } {y } {y } 若 有界，则 必收敛 若 为无穷小，则 必为无穷
> 小 C. D. n n n x n
>
> 题源：900 数二第一章 B 类 9　|　题源类型：900　|　页码：p.71　|　模块：2.3 数列极限
