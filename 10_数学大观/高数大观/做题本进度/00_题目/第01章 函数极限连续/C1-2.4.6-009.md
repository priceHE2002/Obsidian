...
> (−1,1) 设函数 在区间 内有定义，且 limf(x)=0，则（） (2) f(x) x→0 f(x) 
> f(x) p 当 时，f(x) 在 处可导 当 时，f(x) 在 处可导 A. lim x=0 B. lim
>  =0 x=0 |x| x2 x→0 x→0 ·第 页，共 页· 75 402
>
> 题源：2020 数一　|　题源类型：真题　|　页码：p.86　|　模块：2.4 已知极限相关
