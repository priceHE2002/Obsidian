...
> 下列命题 1 ⃝ f(x)=∞，则 设 =0。 lim lim 1 f(x) x→x0 x→x0 1 =∞。
>  ⃝ 设 f(x)=0，则 lim lim 2 f(x) x→x0 x→x0 g(x)=+∞，则 (f(x)
> −g(x))=0。 ⃝ 设 lim lim f(x)= lim 3 x→x0 x→x0 x→x0 ⃝ g(x
> )=+∞，则 (f(x)+g(x))=+∞。 设 lim f(x)= lim lim 4 x→x0 x→x0
>  x→x0 正确的个数为 A. 1 B. 2 C. 3 D. 4
>
> 题源：26 版 660 数三第 620 题　|　题源类型：660　|　页码：p.47　|　模块：2.2 极限计算
