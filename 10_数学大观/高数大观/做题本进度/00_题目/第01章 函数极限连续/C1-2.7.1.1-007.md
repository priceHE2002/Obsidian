...
> 设函数 连续，F(x) 为 的原函数，则下列命题中，错误的是（） f(x) f(x) x→0 当 时，若 f
> (x)=o(1)，则 F(x)=o(x)。 A. x→0 当 时，若 F(x)=o(x)，则 f(x)=o(
> 1)。 B. Z x x→0 当 时，若 f(x)=o(1)，则 f(t)dt=o(x)。 C. Z 0 x
>  x→0 当 时，若 f(t)dt=o(x)，则 f(x)=o(1)。 D. 0
>
> 题源：900 数二第一章 B 类 17　|　题源类型：900　|　页码：p.91　|　模块：2.7 无穷小阶数
