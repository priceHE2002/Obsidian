...
> Z a+(b+2)T 的取值（）。 设 是以 为周期的连续奇函数，则 f(x)dx f(x) T a+(b+
> 1)T 与 均有关 与 均无关 A. a,b,T B. a,b,T 只与 有关 只与 有关 C. T D. 
> a,T 有界
>
> 题源：26 版 1000 第八章强化 2　|　题源类型：1000　|　页码：p.17　|　模块：1.2 函数性质考察
