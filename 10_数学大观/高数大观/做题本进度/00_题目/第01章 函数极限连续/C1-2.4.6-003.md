...
> 设 f(0)=0，则 在点 可导的充要条件为（） f(x) x=0 1 1 f(1−cosh) f(1−eh
> ) 存在。 存在。 A. lim B. lim h→0h2 h→0h 1 1 f(h−sinh) [f(2h
> )−f(h)] 存在。 存在。 D. C. lim lim h→0h2 h→0h
>
> 题源：2001 数一　|　题源类型：真题　|　页码：p.86　|　模块：2.4 已知极限相关
