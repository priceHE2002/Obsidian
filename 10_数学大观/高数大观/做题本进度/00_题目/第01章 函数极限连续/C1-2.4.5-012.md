...
> f′′(x) f′(a)=0 设函数 二阶可导，满足 且 =1，则（ ） f(x) lim x−a x→a 
> 是 的极小值点 A. x=a f(x) 是 的极大值点 B. x=a f(x) 是曲线 的拐点 C. (a,
> f(a)) y =f(x) 不是 的极值点，(a,f(a)) 也不是曲线 的拐点 D. x=a f(x) y
>  =f(x)
>
> 题源：900 第二章数二 A 类 32　|　题源类型：900　|　页码：p.83　|　模块：2.4 已知极限相关
