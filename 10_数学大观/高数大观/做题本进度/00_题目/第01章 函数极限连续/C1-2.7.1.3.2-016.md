...
> x→a x−a 设 时 与 分别是 的 阶与 阶无穷小，则下列命题 f(x) g(x) n m ⃝ x−a 
> 是 的 阶无穷小。 f(x)g(x) n+m 1 f(x) ⃝ x−a n−m 若 n>m，则 是 的 阶无
> 穷小。 2 g(x) ⃝ n⩽m，则 x−a 若 是 的 阶无穷小。 f(xZ)+g(x) n 3 x ⃝ 
> x−a 若 连续，则 是 的 阶无穷小。 f(x) f(t)dt n+1 4 a 中，正确的个数是 A. 1
>  B. 2 C. 3 D. 4
>
> 题源：25 版 660 数一二三第 139 题　|　题源类型：660　|　页码：p.99　|　模块：2.7 无穷小阶数
