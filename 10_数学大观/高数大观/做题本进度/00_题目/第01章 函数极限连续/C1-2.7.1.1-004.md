...
> x→0 (x)∼α (x)∼β 当 时，若 都是非零无穷小量，且 (x)，β (x)，则下列命 α (x),
> α (x),β (x),β (x) α 1 2 1 2 1 2 1 2 题中，错误的是（ ） (x)∼β (
> x)−β 若 (x)，则 A. α α (x)=o(α (x)) 1 1 2 2 2 (x)−β (x)∼β
>  若 (x))，则 B. α (x)=o(α α (x) 1 1 1 2 2 (x)∼β (x)−β (x)
> ∼α (x)−β 若 (x)，则 C. α α (x) 1 1 1 1 2 2 (x)−β (x)∼α (x
> )−β 若 (x))，则 D. α (x)=o(β α (x) 1 1 1 1 2 2
>
> 题源：900 数二第一章 A 类 16　|　题源类型：900　|　页码：p.91　|　模块：2.7 无穷小阶数
