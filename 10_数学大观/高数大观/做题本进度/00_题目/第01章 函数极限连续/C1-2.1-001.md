...
> 下列命题正确的是 设 存在，则必存在 >0，当 A. lim f(x) δ x→x0 设 在 处连续，则必存
> 在 B. f(x) x=x 0 设 为 的间断点，则 C. x=x f(x) 0 设 不存在，则 必不存在.
>  D. f(x ) lim f(x) 0 x→x0
>
> 题源：26 版 660 数二第 509 题　|　题源类型：660　|　页码：p.21　|　模块：2.1 函数极限概念题
