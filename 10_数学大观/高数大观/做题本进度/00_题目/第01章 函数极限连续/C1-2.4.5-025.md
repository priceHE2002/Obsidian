...
> [0,+∞) 设函数 在区间 上可导，则（ ） f(x) ′ ′ 当 存在时， 存在。 当 存在时， 存在。
>  A. lim f(x) lim f (x) B. lim f (x) lim f(x) x→+∞R x→+
> ∞ x→+∞ x→+∞R x x f(t)dt f(t)dt 0 0 存在时， 存在。 当 存在时， 存在。
>  当 lim f(x) D. lim f(x) lim C. lim x x x→+∞ x→+∞ x→+∞ 
> x→+∞ 极限与导数互推
>
> 题源：2025 数一　|　题源类型：真题　|　页码：p.85　|　模块：2.4 已知极限相关
