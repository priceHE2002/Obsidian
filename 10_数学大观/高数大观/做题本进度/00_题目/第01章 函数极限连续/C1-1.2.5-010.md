...
> 1 ，则下列关于 的命题中，正确的是（） 设函数 f(x) f(x)= lim arctan 1+sinnx
>  n→∞ ⃝ 是周期为 的奇函数. f(x) 2π 1 ⃝ 有无穷多个可去间断点. f(x) 2 ⃝ 有无穷
> 多个跳跃间断点. f(x) 3 π ⃝ 的最小值小于 f(x) . 4 6 ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ 
> A. 2. B. 4. C. 4. D. 4. 1 3 1 2
>
> 题源：900 数二第一章 B 类 18　|　题源类型：900　|　页码：p.20　|　模块：1.2 函数性质考察
