...
> (cid:20)(cid:18) n 1 −e nx 在 处 f(x)= lim 1+ x=1 ( ). n
>  n→∞ 左极限存在, 右极限不存在 左极限不存在, 右极限存在 A. B. 左、右极限都存在, 但不相等 
> 连续 C. D. ·第 页，共 页· 78 402
>
> 题源：26 版 1000 第一章强化 13　|　题源类型：1000　|　页码：p.89　|　模块：2.5 以极限形式给出的函数
