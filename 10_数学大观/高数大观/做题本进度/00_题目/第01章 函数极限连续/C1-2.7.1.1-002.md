...
> x→0 若当 时，α(x),β(x) 是非零无穷小量，则以下的命题中 (1) ⃝ α(x)∼β(x)，则 α
> 2(x)∼β2(x)； 若 1 ⃝ α2(x)∼β2(x)，则 α(x)∼β(x)； 若 2 ⃝ α(x)∼
> β(x)，则 α(x)−β(x)=o(α(x))； 若 3 ⃝ α(x)−β(x)=o(α(x))，则 α(
> x)∼β(x)， 若 4 真命题的序号为（ ） ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ A. 3. B. 4
> . C. 4. D. 4. 1 1 1 3 2 3
>
> 题源：2022 数二三　|　题源类型：真题　|　页码：p.90　|　模块：2.7 无穷小阶数
