...
> 以下函数 以 为第二类间断点的是 f(g(x)) x=0 ( 1−u, u⩽0 ，g(x)=2cosx−1 
> f(u)=ln(1+u2)， A. B. f(u)= ( u2+1, u>0 x⩽0 sin2x+(x+1)
> 2, g(x)= x2+1, x>0 8 1 8 >>><x , x<0 <ln(1−u2) 1 sin ,
>  u<0 f(u)=eu2 ， +1，g(x)= C. f(u)= D. u u 0, x=0 √ : >>
> >: 1−c u⩾0 os u, 1 8 sin , x>0 x < x, x<0 g(x)= π2 : x
> ⩾0 x+ , 4
>
> 题源：25 版 660 数一二三第 141 题　|　题源类型：660　|　页码：p.119　|　模块：3.3 间断点
