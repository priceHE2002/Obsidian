...
> (0,+∞) 设函数 在 内有界且可导，则 (4) y =f(x) ( ). ′ ′ ′ 当 时，必有 当 
> 存在时，必有 A. lim f(x)=0 lim f (x)=0 B. lim f (x) lim f (x
> )=0 x→+∞ x→+∞ x→+∞ x→+∞ ′ ′ 当 时，必有 当 存在时，必有 C. lim f(x
> )=0 lim f (x)=0 D. lim f(x) lim f (x)=0 x→+∞ x→0+ x→0+
>  x→0+
>
> 题源：2002 数二　|　题源类型：真题　|　页码：p.85　|　模块：2.4 已知极限相关
