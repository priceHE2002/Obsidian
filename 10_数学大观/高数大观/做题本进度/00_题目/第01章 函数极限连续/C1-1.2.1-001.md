...
> π x∈ 【2】设函数 f(x)=cos(sinx)，g(x)=sin(cosx)，则当 时，（ ）。 0,
>  2 单调增加，g(x) 单调减少 单调减少，g(x) 单调增加 A. f(x) B. f(x) 与 都单调
> 增加 与 都单调减少 C. f(x) g(x) D. f(x) g(x)
>
> 题源：880 第一章基础选择 2　|　题源类型：880　|　页码：p.13　|　模块：1.2 函数性质考察
