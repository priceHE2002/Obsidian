...
> 有以下命题：设 f(x)=A，lim 不存在，lim 不存在， lim g(x) h(x) x→a x→a 
> x→a ⃝ lim(f(x)·g(x)) 不存在。 1 x→a ⃝ 不存在。 lim(g(x)+h(x)) 
> 2 x→a ⃝ lim(h(x)·g(x)) 不存在。 3 x→a ⃝ 不存在。 lim(g(x)+f(x)
> ) 4 x→a 则以上命题中正确的个数是 A. 0 B. 1 C. 2 D. 3
>
> 题源：26 版 660 数一二三第 135 题　|　题源类型：660　|　页码：p.46　|　模块：2.2 极限计算
