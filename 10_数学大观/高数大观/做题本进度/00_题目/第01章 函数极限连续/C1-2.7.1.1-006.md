...
> 下列命题正确的是 设在 空心邻域 为有界函数，且 α(x)β(x)=0，则 A. x=x α(x) lim 
> lim β(x)=0 0 x→x0 x→x0 α(x) x→x =a̸=0，则 β(x)=∞ 设 时 为无穷
> 小，且 B. α(x) lim lim 0 β(x) x→x0 x→x0 x→x α(x)β(x)=a，则 
> 设 时 为无穷大，且 β(x)=0 lim C. α(x) lim 0 x→x0 x→x0 设在 空心邻域 
> 为无界函数，且 α(x)β(x)=0，则 D. x=x α(x) lim lim β(x)=0 0 x→x0
>  x→x0
>
> 题源：26 版 660 数二第 515 题　|　题源类型：660　|　页码：p.91　|　模块：2.7 无穷小阶数
