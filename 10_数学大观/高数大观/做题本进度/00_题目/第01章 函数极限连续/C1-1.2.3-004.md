...
> Z x(cid:2) f(t)−f(−t) 设 为以 为周期的非零连续函数，Φ(x)= dt，a 是常数，则
>  f(x) T a 是以 为周期的偶函数. 是以 为周期的奇函数. Φ(x) B. Φ(x) T A. T 
> 是偶函数，但不一定以 为周期. 是奇函数，但不一定以 为周期. C. Φ(x) T D. Φ(x) T
>
> 题源：25 版 660 数一二三第 196 题　|　题源类型：660　|　页码：p.16　|　模块：1.2 函数性质考察
