...
> x−f(x−1)−1 设函数 在 的某邻域内有定义，且 =1，则以下结论： f(x) x=0 lim lnx
>  x→1 ⃝ f(0)=0； 1 ⃝ limf(x)=0； 2 x→0 f(x) ⃝ =1； lim 3 x
>  x→0 ⃝ x→0 当 时，f(x) 是 的高阶无穷小。 x 4 所有正确结论的序号为（ ）。 ⃝ ⃝ ⃝
>  ⃝ ⃝ ⃝ ⃝ ⃝ A. B. C. D. 1 2 2 4 3 4 2 3 ·第 页，共 页· 72 40
> 2
>
> 题源：26 版 1000 第一章基础 6　|　题源类型：1000　|　页码：p.83　|　模块：2.4 已知极限相关
