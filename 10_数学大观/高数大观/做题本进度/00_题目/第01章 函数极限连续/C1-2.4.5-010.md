...
> f(x) (−1,1) 【7】设 在 内可导，且 =1，则（ ）. f(x) lim x2 x→0 f′(x
> ) f′(x) 存在 不存在 A. lim B. lim x x x→0 x→0 f′(0)=0，f′′(0
> )=2 是 的极小值 C. D. f(0) f(x)
>
> 题源：880 第二章综合选择 7　|　题源类型：880　|　页码：p.83　|　模块：2.4 已知极限相关
