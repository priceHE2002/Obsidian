...
> f′(x) =−1，则 设 的导数在 处连续，又 (1) f(x) x=a lim ( ) x→ax−a 是
>  的极小值点. A. x=a f(x) 是 的极大值点. B. x=a f(x) 是曲线 的拐点. C. (
> a,f(a)) y =f(x) 不是 的极值点，(a,f(a)) 也不是曲线 的拐点. D. x=a f(x
> ) y =f(x)
>
> 题源：2001 数三　|　题源类型：真题　|　页码：p.82　|　模块：2.4 已知极限相关
