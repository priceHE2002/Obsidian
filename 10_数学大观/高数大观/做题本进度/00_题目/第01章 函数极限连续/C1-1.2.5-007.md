...
> 下列命题中，正确的是（ ） 若 与 都不是连续函数，则 也不是连续函数。 A. f(x) g(x) f(x)
> +g(x) 若 与 都不是单调函数，则 也不是单调函数。 B. f(x) g(x) f(x)+g(x) 若 
> 与 都是无界函数，则 也是无界函数。 C. f(x) g(x) f(x)+g(x) 若 与 都是奇函数，则 
> 也是奇函数。 D. f(x) g(x) f(x)+g(x)
>
> 题源：900 数二第一章 A 类 3　|　题源类型：900　|　页码：p.20　|　模块：1.2 函数性质考察
