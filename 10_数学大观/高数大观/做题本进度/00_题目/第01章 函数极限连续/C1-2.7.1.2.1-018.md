...
> Z x ′ −xsin2x，且 f(0)=f′(0)=0，则当 x→0 设函数 满足 时，与 为 f(x) 
> f(t)dt+2f(x)+f (x)=e f(x) 0 同阶但不等价的无穷小量为（） √ ex2 1−cos
> 2x 1+x−1 −1 A. sin2x B. C. D.
>
> 题源：900 数二第一章 B 类 15（重点）　|　题源类型：900　|　页码：p.94　|　模块：2.7 无穷小阶数
