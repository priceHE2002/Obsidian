...
> f′(x) 0，f′′(x) ∈ {x } 【5】设 在 上有二阶导数，且 0，f(0) 0。取 (0,1)
> ，数列 满足 f(x) [0,1] > > = x 1 n )f′(x −x (n=1,2,···)。证明 
> 存在，并求其值。 (x )+f(x )=0 lim x n+1 n n n n n→∞ ▶2.2.2.3.2
>  二阶递推数列求极限
>
> 题源：880 第一章拓展 5（选做）　|　题源类型：880　|　页码：p.64　|　模块：2.2 极限计算
