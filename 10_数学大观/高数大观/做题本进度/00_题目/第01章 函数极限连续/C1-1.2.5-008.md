...
> f(x+4)−f(x)=f(2)，则下列结论中，正确的个数为（ 设函数 满足 f(x) ⃝ 若 为奇函数，则
>  为周期函数。 f(x) f(x) 1 ⃝ 若 为偶函数，则 为周期函数。 f(x) f(x) 2 ⃝ 若 
> 为周期为 的函数，则 为奇函数。 f(x) 4 f(x) 3 ⃝ 若 为周期为 的函数，则 为偶函数。 f(
> x) 4 f(x) 4 A. 1 B. 2 C. 3 D. 4
>
> 题源：900 数二第一章 B 类 4　|　题源类型：900　|　页码：p.20　|　模块：1.2 函数性质考察
