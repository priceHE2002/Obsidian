...
> Z Z x sinx et2 ecostdt，g(x)= 设 dt，则下列正确的是 f(x)= 0 0 为奇
> 函数，g(x) 为偶函数 为偶函数，g(x) 为奇函数 A. f(x) B. f(x) 均为奇函数 均为周期
> 函数 C. f(x),g(x) D. f(x),g(x)
>
> 题源：2024 数一　|　题源类型：真题　|　页码：p.14　|　模块：1.2 函数性质考察
