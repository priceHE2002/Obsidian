...
> f′′(x) f′(0)=0，lim 设 有二阶连续导数，且 =1，则（ ） (2) f(x) |x| x→
> 0 是 的极大值。 A. f(0) f(x) 是 的极小值。 B. f(0) f(x) 是曲线 的拐点。 C
> . (0,f(0)) y =f(x) 不是 的极值，(0,f(0)) 也不是曲线 的拐点。 D. f(0) 
> f(x) y =f(x)
>
> 题源：1996 数一　|　题源类型：真题　|　页码：p.82　|　模块：2.4 已知极限相关
