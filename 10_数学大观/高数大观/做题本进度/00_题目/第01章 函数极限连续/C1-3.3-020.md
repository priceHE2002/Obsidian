...
> 8 <f(x) x̸=0, , f′(0)̸=0，f(0)=0，则（ 【5】设 其中 在 处可导，且 ） x
>  F(x)= f(x) x=0 : f(0), x=0, 是 的连续点 是 的第一类间断点 A. x=0 F
> (x) B. x=0 F(x) 是 的第二类间断点 以上说法均错误 C. x=0 F(x) D.
>
> 题源：880 第一章综合选择 5　|　题源类型：880　|　页码：p.117　|　模块：3.3 间断点
