...
> 1 (−∞,+∞) x∈(−∞,+∞)，均有 f(x+1)−f(x)=f(1)，f 设 是 上可导的奇函数，
> 任意的 =0， f(x) 2 则以下是偶函数的是（ ）. Z Z x x ′ ′ A. [sinf(t)+f
> (t+1)]dt B. [sinf (t)+f (t+1)]dt Z0 Z0 x x ′ ′ C. [cos
> f(t)+f(t+2)]dt D. [cosf (t)+f (t+2)]dt 0 0
>
> 题源：25 版 1000 第八章基础 10　|　题源类型：1000　|　页码：p.14　|　模块：1.2 函数性质考察
