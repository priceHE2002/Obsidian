...
> x→0 时，ef(x)−1 x−ln(1+x) 设 连续，当 与 是等价无穷小量，以下结论： f(x) ⃝ 
> 在 处取得极大值； f(x) x=0 1 ⃝ 是曲线 的拐点； (0,f(0)) f(x) 2 1 ⃝ x2
> . 在 处的 次泰勒多项式为 f(x) x=0 2 3 2 正确结论的个数为（ ）. A. 0 B. 1 C
> . 2 D. 3
>
> 题源：26 版 1000 第五章强化 10　|　题源类型：1000　|　页码：p.85　|　模块：2.4 已知极限相关
