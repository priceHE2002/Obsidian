...
> (−∞,+∞) 设 与 在 内都有定义，且 是 的唯一间断点，x 是 的唯一间断 f(x) g(x) x =
>  x f(x) = x g(x) 1 2 点。则 当 时，f(x)+g(x) 必有唯一的间断点 A. x =
> x x=x 1 2 1 ̸=x 当 时，f(x)+g(x) 必有两个间断点 与 B. x x=x x=x 1
>  2 1 2 当 时，f(x)g(x) 必有唯一的间断点 C. x =x x=x 1 2 1 ̸=x 当 时
> ，f(x)g(x) 必有两个间断点 与 D. x x=x x=x 1 2 1 2
>
> 题源：26 版 660 数二第 520 题　|　题源类型：660　|　页码：p.119　|　模块：3.3 间断点
