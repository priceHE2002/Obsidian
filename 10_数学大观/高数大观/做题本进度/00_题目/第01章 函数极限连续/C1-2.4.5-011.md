...
> f′′(x) 1 f′(1)=0，lim 设 具有二阶连续导数，且 ，则 f(x) = x→1(x−1)2 
> 2 是 的极大值。 A. f(1) f(x) 是 的极小值。 B. f(1) f(x) 是曲线 的拐点坐标。
>  C. (1,f(1)) f(x) 不是 的极值，(1,f(1)) 不是曲线 的拐点坐标。 D. f(1) 
> f(x) f(x)
>
> 题源：25 版 660 数一二三第 160 题　|　题源类型：660　|　页码：p.83　|　模块：2.4 已知极限相关
