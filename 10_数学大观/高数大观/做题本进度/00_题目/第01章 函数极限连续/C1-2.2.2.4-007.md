...
> 1 |f′(x)|<1，当 x∈[a,b] 【1】设 在 上可导，且 时，有 a<f(x)<b，F(x)= 
> [x+f(x)]，证明： f(x) [a,b] 2 x∗ F(x∗)=x∗； ∈(a,b)，使得 存在 (I
> ) ∗。 ∈[a,b]，数列 {x } )(n=0,1,2,···)，有 对 满足 (II) x x =F(
> x lim x =x n+1 0 n n n n→∞
>
> 题源：880 第一章拓展 1　|　题源类型：880　|　页码：p.65　|　模块：2.2 极限计算
