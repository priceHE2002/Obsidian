...
> 下列叙述错误的是 [−a,a] [−a,a] 设 在 上连续为奇函数，则 在 上的全体原函数为偶函数. A.
>  f(x) f(x) [−a,a] [−a,a] 设 在 上连续为偶函数，则 在 上的全体原函数为奇函数. 
> B. f(x) f(x) Z x (−∞,+∞) 设 在 内连续，以 为周期且为奇函数，则 也是以 为周期的
> 函数. C. f(x) T f(t)dt T Z Z 0 +∞ x (−∞,+∞) 设 在 内连续，以 为周
> 期，又 收敛，则 也是以 为周期的 D. f(x) T f(x)dx f(t)dt T 0 0 函数.
>
> 题源：25 版 660 数一二三第 195 题　|　题源类型：660　|　页码：p.16　|　模块：1.2 函数性质考察
