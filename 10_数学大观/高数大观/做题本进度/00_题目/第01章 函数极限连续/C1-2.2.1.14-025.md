...
> 1 已知数列{x }，{y }满足x =1−cosx =y2（n=1,2,···），则当n→∞时，（ ，x 
> ，y =y = n n 1 1 n+1 n n+1 n 2 ） −y x x +y n n n n A. l
> im =0 B. lim =1 y y n→∞ n→∞ n n −y x x +y n n n n C. l
> im =0 D. lim =1 x x n→∞ n→∞ n n 数列极限
>
> 题源：900 数二第一章 B 类 7（选做）　|　题源类型：900　|　页码：p.50　|　模块：2.2 极限计算
