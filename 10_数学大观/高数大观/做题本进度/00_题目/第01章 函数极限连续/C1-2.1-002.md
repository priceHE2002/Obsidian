...
> 【例 1.6】设 f(x)， lim lim g(x) x→x0 x→x0 若 g(x)，则存在 A. li
> m f(x)> lim x→x0 x→x0 f(x)≥ g(x)，则存在 若 lim B. lim x→x0
>  x→x0 0<|x−x |<δ 若存在 >0，当 C. δ 0 0<|x−x |<δ 若存在 >0，当 D
> . δ 0
>
> 题源：姜晓千基础例题（选做）　|　题源类型：其他　|　页码：p.21　|　模块：2.1 函数极限概念题
