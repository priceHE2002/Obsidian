...
> 1 设函数 ，则（ ） f(x)= (x− −3) 2) (x −1 e − x 1 x=1，x=2，x=3
>  都是 的第一类间断点 A. f(x) x=1，x=2，x=3 都是 的第二类间断点 B. f(x) 是 的
> 第一类间断点，x=2，x=3 是 的第二类间断点 C. x=1 f(x) f(x) 是 的第二类间断点，x=
> 2，x=3 是 的第一类间断点 D. x=1 f(x) f(x)
>
> 题源：900 数二第一章 A 类 23　|　题源类型：900　|　页码：p.118　|　模块：3.3 间断点
