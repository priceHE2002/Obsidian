...
> (−∞,+∞) −4)，若对任意的 x(x2 设函数 在 内有定义，在区间 上，f(x) 都满足 f(x) 
> [0,2] = x f(x) = 1 − [−2,0) f(x+2)。写出 在 上的表达式。 f(x) 2
>
> 题源：26 版 1000 第一章基础 2　|　题源类型：1000　|　页码：p.13　|　模块：1.1 求函数表达式
