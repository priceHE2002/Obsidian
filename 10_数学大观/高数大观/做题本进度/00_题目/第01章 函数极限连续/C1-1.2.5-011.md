...
> Z x (x−2t)f(x−t)dt，其中 设函数 为连续函数，则下列命题中，正确的是（ ） F(x)= f
> (x) 0 ⃝ 若 为单调减少函数，则 为单调减少函数。 f(x) F(x) 1 ⃝ 若 为单调减少函数，则
>  为单调增加函数。 f(x) F(x) 2 ⃝ 若 为奇函数，则 为奇函数。 f(x) F(x) 3 ⃝ 若
>  为奇函数，则 为偶函数。 f(x) F(x) 4 ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ ⃝ A. B. C. D. 
> 1 3 1 4 2 3 2 4
>
> 题源：900 第三章数二 A 类 19　|　题源类型：900　|　页码：p.20　|　模块：1.2 函数性质考察
