...
> >0(n=1,2,···) {u } 设 并设数列 无上界，则 u n(cid:26) n 1 数列 必有上
> 界。 A. u n =+∞。 必有 B. lim u n n→∞ 对于任意给定的 >0，满足 的 只有有限个
> 。 C. M u <M n n 对于任意给定的 >0，满足 的 总有无限个。 D. M u >M n n 数
> 列极限最值
>
> 题源：26 版 660 数三第 621 题　|　题源类型：660　|　页码：p.71　|　模块：2.3 数列极限
