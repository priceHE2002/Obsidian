...
> {a },{b },{c } =∞，则必有（ （1）设 均为非负数列，且 =0， =1， ） lim a l
> im b lim c n n n n n n n→∞ n→∞ n→∞ 对任意 成立 对任意 成立 A. a 
> <b n B. b <c n n n n n 极限 不存在 极限 不存在 C. lim a c D. lim
>  b c n n n n n→∞ n→∞
>
> 题源：2003 数一二　|　题源类型：真题　|　页码：p.66　|　模块：2.3 数列极限
