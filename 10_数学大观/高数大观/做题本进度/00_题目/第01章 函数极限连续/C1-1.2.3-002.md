...
> f′(x)，则设 f′(x) 【2】设 是以 为周期的连续函数（若下式中用到 存在），则以下结论中不正确的是
>  f(x) T （ ）． Z x f′(x) 必以 为周期 必以 为周期 A. T B. f(t)dt T 
> Z0 Z Z T x x x [f(t)−f(−t)]dt f(t)dt− 必以 为周期 必以 为周期 f(
> t)dt T C. T D. T 0 0 0
>
> 题源：25 版 1000 第八章强化 2　|　题源类型：1000　|　页码：p.16　|　模块：1.2 函数性质考察
