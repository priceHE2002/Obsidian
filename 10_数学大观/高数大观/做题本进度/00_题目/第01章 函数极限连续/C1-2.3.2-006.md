...
> f(x)=e−|x|，则（ {x }，其中 −∞<x <+∞，函数 已知数列 ） n n 当 存在时， 存在
>  A. lim arctanf(x ) lim x n n n→∞ n→∞ 当 存在时， 存在 B. lim
>  f(arctanx ) lim x n n n→∞ n→∞ 当 存在时， 存在，但 不一定存在 C. li
> m arctanf(x ) lim f(x ) lim x n n n n→∞ n→∞ n→∞ 当 存在时，
>  存在，但 不一定存在 D. lim f(arctanx ) lim arctanx lim x n n n
>  n→∞ n→∞ n→∞
>
> 题源：900 数二第一章 B 类 8　|　题源类型：900　|　页码：p.68　|　模块：2.3 数列极限
