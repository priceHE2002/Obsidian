...
> f(0)̸=0，f′(0)̸=0，f′′(0)̸=0。证明：存在唯一 设函数 在 的某邻域内具有二阶连续导数
> ，且 f(x) x=0 h→0 f(3h)−f(0) h2 的一组实数 ，使得当 时，λ 是比 高阶的无穷小
> 。 λ ,λ ,λ f(h)+λ f(2h)+λ 1 2 3 1 2 3
>
> 题源：2002 数二　|　题源类型：真题　|　页码：p.96　|　模块：2.7 无穷小阶数
