...
> 设 是连续函数，F(x) 是 的原函数，则（ ）。 (2) f(x) f(x) 当 为奇函数时，F(x) 必
> 为偶函数 当 为偶函数时，F(x) 必为奇函数 A. f(x) B. f(x) 当 为周期函数时，F(x) 
> 必为周期函数 当 为单调函数时，F(x) 必为单调函数 C. f(x) D. f(x)
>
> 题源：880 第三章基础选择 2　|　题源类型：880　|　页码：p.15　|　模块：1.2 函数性质考察
