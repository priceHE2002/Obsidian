...
> g(x)=B，则下列命题不正确的是 f(x)=A， 设 lim lim x→x0 x→x0 [f(x)−g(
> x)]=A−B A. lim [f(x)+g(x)]=A+B B. lim x→x0 x→x0 f(x) A
>  [f(x)·g(x)]=AB C. lim D. lim = g(x) B x→x0 x→x0
>
> 题源：26 版 660 数二第 508 题　|　题源类型：660　|　页码：p.46　|　模块：2.2 极限计算
