...
> 【例 1.6】（I）设 连续，且 f(x),g(x) Z φ(x) g(t)dt； 0 Z x2 ln(1+
> 2tant)dt (cid:20)Z0 （II）求极限 lim x x→0 ln(1+2tant)dt 0 
> f(x) （III）设 连续，且 =1，求极限 f(x) lim x x→0
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.42　|　模块：2.2 极限计算
