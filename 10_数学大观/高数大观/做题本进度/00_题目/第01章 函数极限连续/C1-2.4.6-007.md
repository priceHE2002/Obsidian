...
> 设函数 在 处连续，以下命题错误的选项是（ ） (2) f(x) x=0 f(x)+f(−x) f(x) 假
> 设 存在，则 假设 存在，则 A. lim f(0)=0 B. lim f(0)=0. x x x→0 x→
> 0 f(x)−f(−x) f(x) f′(0) f′(0) 假设 存在，则 存在 假设 存在，则 存在. C
> . lim D. lim x x x→0 x→0
>
> 题源：2007 数一二三　|　题源类型：真题　|　页码：p.86　|　模块：2.4 已知极限相关
