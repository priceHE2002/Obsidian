...
> 设函数 满足 f(0)=0，则 在 处可导的充分必要条件为（） f(x) f(x) x=0 1 1 f(ta
> nh−h) f(ln(1+h)−h) 存在 存在 A. lim B. lim h→0h3 h→0h2 1 1
>  f(arctanh−h) [f(h)−f(−h)] 存在 存在 C. lim D. lim h→0h h→
> 0h
>
> 题源：900 第二章数二 B 类 2　|　题源类型：900　|　页码：p.86　|　模块：2.4 已知极限相关
