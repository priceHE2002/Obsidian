...
> {x } 设数列 收敛，则 (3) ( ) n p |x |)=0 当 时， 当 时， A. lim sin
> x =0 lim x =0. B. lim (x + lim x =0. n n n n n n→∞ n→∞
>  n→∞ n→∞ +x2)=0 当 时， 当 时， C. lim (x lim x =0. D. lim (
> x +sinx )=0 lim x =0. n n n n n n n→∞ n→∞ n→∞ n→∞
>
> 题源：2017 数二　|　题源类型：真题　|　页码：p.68　|　模块：2.3 数列极限
