...
> {a } {a } 已知数列 和连续函数 若 )，且 1，则下列条件中，可以推出” 数列 收 f(x). b
>  = f(a lim b = n n n n n n→∞ 敛” 的条件个数是（） ⃝ 是单调函数. f(x)
>  1 ⃝ 是偶函数. f(x) 2 ⃝ 是有界函数. f(x) 3 ⃝ 是周期函数. f(x) 4 A. 0
>  B. 1 C. 2 D. 3
>
> 题源：900 数二第一章 B 类 2　|　题源类型：900　|　页码：p.70　|　模块：2.3 数列极限
