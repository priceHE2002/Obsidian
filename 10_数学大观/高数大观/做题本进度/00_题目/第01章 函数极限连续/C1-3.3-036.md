...
> 下面结论正确的是 ( ). 设 在 处连续，而 在 处不连续，则函数 在 处一定不连续 A. u=f(x) 
> x v =g(x) x y =f(x)g(x) x 0 0 0 设 在 处连续，而 在 处不连续，则函数 在
>  处一定不连续 B. u=f(x) x v =g(x) x y =f(x)+g(x) x 0 0 0 设 在
>  处连续，而 在 处不连续，则复合函数 在 处一定不 C. u=f(x) x v =g(u) u =f(x 
> ) y =g[f(x)] x 0 0 0 0 连续 以上选项均不正确 D. ·第 页，共 页· 108 40
> 2
>
> 题源：26 版 1000 第一章强化 39　|　题源类型：1000　|　页码：p.119　|　模块：3.3 间断点
