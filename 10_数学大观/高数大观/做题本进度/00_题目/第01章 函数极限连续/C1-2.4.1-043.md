...
> f−1(2t) f(x)=2x3+x2+1，x>0，x=f−1(y) √ 设函数 为 的反函数，若正整数 满
> 足 存在且 f(x) k lim t→+∞ kt 非零，则 k = 已知一极限，求另一极限
>
> 题源：900 数二第一章 B 类 21（选做）　|　题源类型：900　|　页码：p.77　|　模块：2.4 已知极限相关
