...
> |f(x)|−f(0) f(x)−|f(0)| |f(x)| 设函数f(x)连续，给出下列4个条件：⃝ 存在
> ；⃝ 存在；⃝ lim lim lim 1 2 3 x x x x→0 x→0 x→0 |f(x)|−|f(
> 0)| 存在；⃝ 存在。其中可得到“f(x) 在 处可导”的条件个数为（ ） lim x=0 4 x x→0
>  A. 1 B. 2 C. 3 D. 4
>
> 题源：2025 数二; 26 版 1000 第三章强化 24　|　题源类型：真题　|　页码：p.86　|　模块：2.4 已知极限相关
