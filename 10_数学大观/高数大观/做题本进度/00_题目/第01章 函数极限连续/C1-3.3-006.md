...
> 设函数 8 3 ln (1 + a x ) >>>>>>>< , x<0, − x a rc si n x 
> 6, x=0, f(x)= >>>>>>>: eax+x2− ax−1 , x>0. x xsin 4 问 
> 为何值时，f(x) 在 处连续？a 为何值时，x=0 是 的可去间断点？ a x=0 f(x)
>
> 题源：2003 数二　|　题源类型：真题　|　页码：p.115　|　模块：3.3 间断点
