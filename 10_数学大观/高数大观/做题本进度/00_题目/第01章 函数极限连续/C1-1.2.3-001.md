...
> 【例Z1.1】设 为以 为周期的连续函数，则下列结论正确的个数是【 】 f(x) T x ⃝ 以 为周期 f
> (t)dt T 1 Z0 Z x T x ⃝ f(t)dt− 以 为周期 f(t)dt T 2 T Z 0 
> 0 x ⃝ 若 为奇函数，则 以 为周期 f(x) f(t)dt T 3 Z 0 x(cid:2) ⃝ f(
> t)−f(−t) 以 为周期 dt T 4 Z Z 0 +∞ x ⃝ 若 收敛，则 以 为周期 f(x)dx
>  f(t)dt T 5 0 0 A. 1 B. 2 C. 3 D. 4
>
> 题源：姜晓千真题同源 150（重点）　|　题源类型：真题同源　|　页码：p.16　|　模块：1.2 函数性质考察
