...
> [0,+∞) 【2】(I) 设 是 上单调减少且非负的连续函数. 证明: f(x) Z k+1 f(k+1)
> ⩽ f(x)dx⩽f(k) =1,2,···); (k k 1 1 +···+ 1+ 1 1 2 n ln(
> 1+n)⩽1+ +···+ ⩽1+lnn, 证明: 并求极限 (II) lim . 2 n lnn n→∞
>
> 题源：880 第一章拓展 2　|　题源类型：880　|　页码：p.65　|　模块：2.2 极限计算
