...
> ′ ′ 【22】设 (x)=1，则（ ）。 lim f (x)= lim f x→x− x→x+ 0 0 f
> ′(x 在 处必可导且 在 处必连续但不可导 A. f(x) x=x )=1 B. f(x) x=x 0 0
>  0 在 处必存在极限但不连续 在 处的某去心邻域内单调递增 C. f(x) x=x D. f(x) x=x
>  0 0
>
> 题源：880 第二章综合选择 22　|　题源类型：880　|　页码：p.84　|　模块：2.4 已知极限相关
