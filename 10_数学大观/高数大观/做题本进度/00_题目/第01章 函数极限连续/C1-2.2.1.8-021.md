...
> f′(0) 0，f′′(x) 设 有二阶连续导数，且 0，又设 是曲线 在点 f(x) f(0) = = >
>  u = u(x) y = f(x) (x,f(x)) x 处的切线在 轴上的截距，则 x lim = x→
> 0u(x) 1 A. 1 B. 2 C. D. 0 2
>
> 题源：26 版 660 数二第 553 题　|　题源类型：660　|　页码：p.38　|　模块：2.2 极限计算
