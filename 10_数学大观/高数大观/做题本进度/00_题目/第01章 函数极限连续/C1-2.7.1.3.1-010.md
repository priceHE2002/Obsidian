...
> 1 x→0+ 时，arcsinαx，(x−sinx)α 已知当 均为比 高阶的无穷小量，且函数 x 8 ar
> csinαx >< , x>0, 1 xβ(x−sinx)α f(x)= >: x⩽0, γ, 若 为整数，
> 且 连续，求 αβγ。 α f(x) ▶2.7.1.3.2 已知同阶
>
> 题源：900 数二第一章 A 类 59　|　题源类型：900　|　页码：p.97　|　模块：2.7 无穷小阶数
