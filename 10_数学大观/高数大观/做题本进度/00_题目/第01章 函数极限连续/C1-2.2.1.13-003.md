...
> 以下极限等式 (若某端极限存在, 则另一端极限也存在且相等) 成立的是 f (x)+f (x) f (x)+
> f (x) 1 2 1 2 设 则 A. lim h(x)=A, lim = lim h(x)+g(x) A
> +g(x) x→a x→a(cid:18) x→a f(x) h(x)· 设 则 B. lim h(x)=0
> , lim =0 g(x) x→a x→a 1 f (x)+f (x) f (x)+f (x) 1 2 1 
> 2 h(x)=A̸=0, 设 则 = lim C. lim lim h(x)g(x) Ax→a g(x) x
> → x→a (cid:18)a sinx f(x) sinx f(x) D. lim + = lim + l
> im x2 x2 x x x→0 x→0 x→0
>
> 题源：26 版 660 数二第 507 题　|　题源类型：660　|　页码：p.46　|　模块：2.2 极限计算
