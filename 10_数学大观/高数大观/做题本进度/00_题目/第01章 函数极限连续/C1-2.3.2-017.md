...
> p 已知函数：⃝ 1f(x)=2x；⃝ 2f(x)=x2；⃝ |x|；⃝ 4f(x)=arctanx，则满足
> 对于任意的首项 ，数 3f(x)= a 1 )(n=2,3,···) 列 均收敛的函数个数为（） a =f(
> a n−1 n A. 1 B. 2 C. 3 D. 4 数列有界
>
> 题源：900 数二第一章 B 类 14　|　题源类型：900　|　页码：p.70　|　模块：2.3 数列极限
