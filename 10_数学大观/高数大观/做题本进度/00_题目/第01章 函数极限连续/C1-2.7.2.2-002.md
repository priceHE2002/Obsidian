...
> f(x)=ln2x，g(x)=x，h(x)=ex 【9】设 2(x>1)，则当 充分大时，（ ） x A. 
> f(x)<g(x)<h(x) B. g(x)<h(x)<f(x) C. h(x)<g(x)<f(x) D. 
> g(x)<f(x)<h(x) ·第 页，共 页· 91 402
>
> 题源：880 第一章基础选择 9　|　题源类型：880　|　页码：p.102　|　模块：2.7 无穷小阶数
