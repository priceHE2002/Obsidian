...
> 8 >< 1 x̸=0 f , (−∞,+∞) x ，则（ ） 设 在 内有定义，且 f(x)=a，g(x)
> = f(x) lim >: x→∞ 0, x=0 必是 的第一类间断点 必是 的第二类间断点 A. x=0 
> g(x) B. x=0 g(x) 必是 的连续点 在点 处的连续性与 的取值有关 C. x=0 g(x) D
> . g(x) x=0 a
>
> 题源：2004 数三　|　题源类型：真题　|　页码：p.115　|　模块：3.3 间断点
