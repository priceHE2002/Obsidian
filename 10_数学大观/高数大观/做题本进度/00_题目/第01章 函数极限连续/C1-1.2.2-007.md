...
> Z a [−a,a] |x−t|·f(t)dt，则在 [−a,a] 设 在 上是连续的偶函数，a>0，g(x
> )= 上（ ）。 f(x) −a 是单调递增函数 是单调递减函数 A. g(x) B. g(x) 是偶函数 
> 是奇函数 C. g(x) D. g(x)
>
> 题源：25 版 1000 第九章基础 15　|　题源类型：1000　|　页码：p.15　|　模块：1.2 函数性质考察
