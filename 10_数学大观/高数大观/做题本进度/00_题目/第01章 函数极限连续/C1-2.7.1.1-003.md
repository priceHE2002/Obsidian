...
> x→0 当 时, 与 是非零且不相等的等价无穷小量, 以下 个结论: α(x) β(x) 4 ⃝ α(x)+
> β(x)=2α(x); 1 ⃝ α(x)+β(x)=2β(x); 2 ⃝ α(x)−β(x)=o(α(x))
> ; 3 ⃝ α(x)−β(x)=o(β(x)). 4 所有正确结论的序号是 ( ). ⃝ ⃝ ⃝ ⃝ ⃝ ⃝
>  ⃝ ⃝ ⃝ ⃝ A. B. C. D. 1 3 3 4 1 2 3 4 2 4 ·第 页，共 页· 79 
> 402
>
> 题源：26 版 1000 第一章强化 24　|　题源类型：1000　|　页码：p.90　|　模块：2.7 无穷小阶数
