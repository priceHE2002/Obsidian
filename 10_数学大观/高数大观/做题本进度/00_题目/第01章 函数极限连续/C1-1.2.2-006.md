...
> Z Z x sinx sint3dt，g(x)= f(t)dt，则（ ） 已知 f(x)= 0 0 为奇函数
> ，g(x) 为奇函数 为奇函数，g(x) 为偶函数 A. f(x) B. f(x) 为偶函数，g(x) 为偶
> 函数 为偶函数，g(x) 为奇函数 C. f(x) D. f(x) ·第 页，共 页· 3 402
>
> 题源：2024 数二　|　题源类型：真题　|　页码：p.14　|　模块：1.2 函数性质考察
