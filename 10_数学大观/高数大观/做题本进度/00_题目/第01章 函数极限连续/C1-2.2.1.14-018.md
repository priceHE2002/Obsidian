...
> √ √ 1 √ x⩾0，由中值定理可知，∃ξ ∈(x,x+1)，使得 x+1− θ(x)=ξ−x，则下列正确
> 的是 设 。若记 x= 2 ξ 1 1 单调增，且 θ(x)=0， 单调减，且 ， A. θ(x) lim 
> lim θ(x)= B. θ(x) lim θ(x)= lim θ(x)=0 4 4 x→+∞ x→+∞ x
> →0+ x→0+ 1 1 1 1 单调增，且 ， 单调减，且 ， C. θ(x) lim θ(x)= lim
>  θ(x)= D. θ(x) lim lim θ(x)= θ(x)= 4 2 2 4 x→+∞ x→+∞ x
> →0+ x→0+
>
> 题源：26 版 660 数一二三第 169 题　|　题源类型：660　|　页码：p.49　|　模块：2.2 极限计算
