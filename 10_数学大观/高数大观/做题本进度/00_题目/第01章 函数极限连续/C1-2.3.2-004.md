...
> {x } 设数列 收敛，则（） n p |x |)=0 (x3 当 时， =0。 当 时， =0。 A. l
> im tanx =0 lim x B. lim + lim x n n n n n n→∞ n→∞ n→∞ 
> n→∞ (x2 +x3)=0 当 时， =0。 当 时， =0。 C. lim lim x D. lim [
> x +ln(x +1)]=0 lim x n n n n n n n→∞ n→∞ n→∞ n→∞
>
> 题源：900 数二第一章 A 类 7　|　题源类型：900　|　页码：p.68　|　模块：2.3 数列极限
