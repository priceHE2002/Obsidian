...
> {x } {y } 设数列 与 满足 =0，则下列断言正确的是（ ） (1) lim x y n n n n
>  n→∞ {x } {y } {x } {y } 若 发散，则 必发散 若 无界，则 必有界 A. B. n
>  n (cid:26)n n 1 {x } {y } {y } 若 有界，则 必为无穷小 若 为无穷小，则 
> 必为无穷小 C. D. n n n x n
>
> 题源：1998 数二　|　题源类型：真题　|　页码：p.66　|　模块：2.3 数列极限
