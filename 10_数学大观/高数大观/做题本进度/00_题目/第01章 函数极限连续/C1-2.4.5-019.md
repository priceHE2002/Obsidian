...
> f′′(a) ̸= ∈ (a−δ,a+δ) 设函数 在 的某个邻域内具有二阶导数，且 0，且存在 0，使得当
>  f(x) x = a δ > x f(t)−f(x) f(t)−f(x) x̸=a ⩾0，则（ 时，lim
>  均存在，且当 时，lim ） (t−x)2 (t−x)2 t→a t→a 是 的极小值点 是 的极大值点 
> A. x=a f(x) B. x=a f(x) 不是 的极值点 无法确定 是否为极值点 C. x=a f(x
> ) D. x=a
>
> 题源：900 第二章数二 A 类 13　|　题源类型：900　|　页码：p.84　|　模块：2.4 已知极限相关
