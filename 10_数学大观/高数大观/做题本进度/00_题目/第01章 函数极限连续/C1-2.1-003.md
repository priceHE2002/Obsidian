...
> [!note]- 题干预览（自动提取，仅供参考）
> 下列命题中正确的是 f(x)⩾ 若 A. lim x→x0 若存在 B. δ >0 。 A >B 0 0 若存在 >0，当 C. δ 若 D. lim f(x)> x→x0 极限计算 2.2 函数极限 2.2.1 ▶2.2.1.1 0/0
>
> 题源：26 版 660 数三第 619 题（选做）　|　页码：p.21　|　模块：2.1 函数极限概念题
