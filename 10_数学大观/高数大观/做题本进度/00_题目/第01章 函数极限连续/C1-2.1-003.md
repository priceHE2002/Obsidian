...
> 下列命题中正确的是 f(x)⩾ g(x)⇒ 若 存在 A. lim lim x→x0 x→x0 0<|x−x
>  |<δ 若存在 使得当 B. δ >0 0 A >B 0 0 0<|x−x |<δ 若存在 >0，当 C.
>  δ 0 g(x)⇒ 若 存在 D. lim f(x)> lim x→x0 x→x0 极限计算
>
> 题源：26 版 660 数三第 619 题（选做）　|　题源类型：660　|　页码：p.21　|　模块：2.1 函数极限概念题
...
→ 所属模块：[[C1-2.1 函数极限概念题|2.1 函数极限概念题]]