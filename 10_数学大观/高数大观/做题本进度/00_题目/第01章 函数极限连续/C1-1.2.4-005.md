...
> (x3−1)sinx ，x∈(−∞,+∞)，x̸=0。则 设 f(x)= (x2+1)x |x|⩽X |x|
> >X 存在某 >0，在 上 无界，但在 上有界。 A. X f(x) |x|⩽X |x|>X 存在某 >0，
> 在 上 有界，但在 上无界。 B. X f(x) |x|⩽X (−∞,+∞) 对任意 >0，在 上 有界，但
> 在 上无界。 C. X f(x) (−∞,+∞) 在 上 有界。 D. f(x)
>
> 题源：26 版 660 数二第 518 题　|　题源类型：660　|　页码：p.18　|　模块：1.2 函数性质考察
