...
> [sinln(1+x)−sinlnx]= . lim . x→+∞ 为连续函数，则 等于（ ） f(x) l
> im F(x) x→a 不存在 C. 0 D. Z x F(x) tn−1f(xn−tn)dt，求 lim 
> x2n x→0 0 9 # du ·第 页，共 页· 29 402
>
> 题源：900 数二第一章 A 类 39　|　题源类型：900　|　页码：p.40　|　模块：2.2 极限计算
