...
> π π {x } − ⩽x ⩽ 设数列 满足 ，则（ ） (6) n n 2 2 若 存在，则 存在。 A.
>  lim cos(sinx ) lim x n n n→∞ n→∞ 若 存在，则 存在。 B. lim si
> n(cosx ) lim x n n n→∞ n→∞ 若 存在，则 存在，但 不一定存在。 C. lim c
> os(sinx ) lim sinx lim x n n n n→∞ n→∞ n→∞ 若 存在，则 存在，但
>  不一定存在。 D. lim sin(cosx ) lim cosx lim x n n n n→∞ n→∞
>  n→∞
>
> 题源：2022 数一二　|　题源类型：真题　|　页码：p.68　|　模块：2.3 数列极限
