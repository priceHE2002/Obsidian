...
> x→+∞ x→+∞ 【4】设当 时，f(x)，g(x) 都是无穷大，则当 时，下列结论正确的是（ ） f(x
> )−g(x) 是无穷小 是无穷大 A. B. f(x)+g(x) g(x) f(x)+g(x) →1 是无穷
> 小 C. D. f(x) f(x)g(x) ·第 页，共 页· 35 402
>
> 题源：880 基础选择 4　|　题源类型：880　|　页码：p.46　|　模块：2.2 极限计算
