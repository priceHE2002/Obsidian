...
> 连接视觉和语言的关键桥梁。在 400M 互联网图文对上用对比学习训练，使模型学会将图像和文本映射到同一语义嵌入空间。CLIP 的视觉编码器成为 VLA 视觉 backbone 的事实标准——[[../15_DINOv2/DINOv2.md|SigLIP]]（CLIP 的改进版）是 OpenVLA 和 π0 的核心视觉编码器。CLIP 也首次展示了零样本图像分类与有监督模型相当的能力，以及对分布偏移的显著鲁棒性。
...
文本编码器使用 [[../03_GPT/GPT.md|GPT-2]] 风格的 Transformer 架构：
...
1. **SigLIP（CLIP 改进版）是 OpenVLA 和 π0 的视觉编码器核心组件**：OpenVLA 使用 SigLIP + [[../15_DINOv2/DINOv2.md|DINOv2]] 双视觉编码器，其中 SigLIP 提供了语义对齐能力
...
- ⚠️ **CLIP 零样本不适用于精细操纵任务**：如果 VLA 需要精确的空间推理（如"在蓝色杯子里放红色方块"），CLIP 的全局匹配能力不足。需要 [[../15_DINOv2/DINOv2.md|DINOv2]] 这样的 patch-level 编码器来补充空间理解
