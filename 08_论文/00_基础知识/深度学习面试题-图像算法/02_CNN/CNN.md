...
定义了卷积的局部感受野。过去常设为 5，如 LeNet-5；现在多设为 3，通过堆叠多个 $3 \times 3$ 卷积核来达到更大的有效感受野。
...
①输入图片每个像素进行一次 full convolution，根据 full convolution 大小计算可以知道每个像素的卷积后大小为 $1+4-1=4$，即 $4 \times 4$ 大小的特征图。输入有 4 个像素，所以会得到 4 个 $4 \times 4$ 的特征图。
