...
- frontmatter 用 `title / tags / type / updated`；H2 用中文数字 `一、二、`；
  ...
  自测用**折叠 callout** `> [!question]- 🎯 自测：…`。
- **折叠一律用 callout 语法，不要用 `<details>`**：`<details>`/`<summary>` 在 Obsidian **实时预览（默认编辑视图）**下
  只展开、点不动，用户会直接反馈「没折叠起来」。要默认收起就在 callout 标题行末加 `-`，默认展开用 `+`。
- **例题 / 真题**：题干与选项保持可见，**答案与解析一律收进折叠 callout**（`> [!success]- ✅ 答案与解析 · 点击展开`，
  标题里不出现答案字母），标题标 `【单选】`/`【多选】`，≥5 道时开头补一张「真题速览表」。
